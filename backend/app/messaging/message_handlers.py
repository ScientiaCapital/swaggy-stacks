...
from app.messaging.rabbitmq_client import (
    MessageType,
    TradingMessage,
    get_rabbitmq_client,
)
...
class TradingMessageHandler:
    ...
    async def handle_trading_signal(self, message: TradingMessage):
        ...
        try:
            ...
            if dashboard_websocket.connection_manager.active_connections:
                ...
                await dashboard_websocket.connection_manager.broadcast_trading_signal(
                    signal_update
                )
    ...
    async def handle_portfolio_update(self, message: TradingMessage):
        ...
        try:
            payload = message.payload
            logger.info(
                "Processing portfolio update", total_value=payload.get("total_value")
            )
            ...
            if dashboard_websocket.connection_manager.active_connections:
                ...
                await dashboard_websocket.connection_manager.broadcast_portfolio_update(
                    portfolio_update
                )

            # Cache portfolio state
            await self.market_cache.set(
                "portfolio_latest", payload, ttl_override=300
            )  # 5 minutes
    ...
    async def handle_market_data(self, message: TradingMessage):
        ...
        try:
            ...
            symbol = payload.get("symbol", "UNKNOWN")
            logger.debug(
                "Processing market data", symbol=symbol, price=payload.get("price")
            )
            ...
            if dashboard_websocket.connection_manager.active_connections:
                ...
                await dashboard_websocket.connection_manager.broadcast_market_update(
                    market_update
                )
    ...
    async def handle_risk_alert(self, message: TradingMessage):
        ...
        try:
            ...
            await self.market_cache.set(
                cache_key, payload, ttl_override=86400
            )  # 24 hours
    ...
    async def handle_system_health(self, message: TradingMessage):
        ...
        try:
            ...
            await self.market_cache.set(
                health_cache_key, payload, ttl_override=300
            )  # 5 minutes
    ...
    async def get_processing_stats(self) -> Dict[str, Any]:
        ...
        return {**self.processing_stats, "timestamp": datetime.now().isoformat()}
    ...
    async def route_message(self, message: TradingMessage):
        ...
        try:
            ...
            if handler:
                ...
                logger.debug(
                    "Message routed successfully", message_type=message.message_type
                )
            else:
                logger.warning(
                    "No handler for message type", message_type=message.message_type
                )
