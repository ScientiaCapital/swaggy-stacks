...
class RabbitMQClient:
    ...
    def __init__(
        ...
        self.connection_url = connection_url or getattr(
            settings, "RABBITMQ_URL", "amqp://localhost:5672/"
        )
    ...
    async def publish_message(
        self, message: TradingMessage, routing_key: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to publish message", error=str(e), routing_key=routing_key
            )
            return False

    async def publish_trading_signal(
        self, signal_data: Dict[str, Any], source_service: str = "trading_engine"
    ) -> bool:
    ...
    async def publish_portfolio_update(
        self, portfolio_data: Dict[str, Any], source_service: str = "portfolio_service"
    ) -> bool:
    ...
    async def publish_market_data(
        self, market_data: Dict[str, Any], source_service: str = "market_data_service"
    ) -> bool:
    ...
    async def publish_risk_alert(
        self, alert_data: Dict[str, Any], source_service: str = "risk_manager"
    ) -> bool:
        ...
        try:
            ...
            async def message_consumer(message: AbstractIncomingMessage):
                async with message.process():
                    try:
                        # Parse message
                        trading_message = TradingMessage.from_json(
                            message.body.decode()
                        )
                        ...
                        logger.error(
                            "Error processing message", error=str(e), queue=queue_name
                        )
            ...
            logger.error(
                "Failed to subscribe to messages", error=str(e), queue=queue_name
            )
            return False
