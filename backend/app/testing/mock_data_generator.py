...
class MockDataGenerator:
    ...
    def generate_market_scenario(
        ...
        logger.info(
            "Generating mock market scenario",
            symbol=symbol,
            regime=regime.value,
        ...
        for i in range(0, duration_minutes, interval_minutes):
            ...
            if regime == MarketRegime.MEAN_REVERSION:
                ...
                price_change = (
                    params["drift"] + mean_revert_factor
                ) * current_price + random.gauss(
                    0, params["volatility"] * current_price
                )
            else:
                # Standard price evolution
                price_change = params["drift"] * current_price + random.gauss(
                    0, params["volatility"] * current_price
                )

            current_price = max(
                current_price + price_change, 0.01
            ...
            base_volume = 1000000
            volume = int(
                base_volume * params["volume_multiplier"] * (1 + random.gauss(0, 0.3))
            )
        ...
        logger.info(
            "Mock market scenario generated",
            symbol=symbol,
            regime=regime.value,
    ...
    def _calculate_technical_indicators(self, symbol: str) -> Dict[str, float]:
        ...
        try:
            ...
            if "ema_12" in indicators and "ema_26" in indicators:
                ...
                indicators["macd_signal"] = macd_line * 0.9 + random.gauss(
                    0, abs(macd_line) * 0.1
                )
            ...
            if len(prices) >= 14:
                high_low = np.array(
                    [
                        max(prices[i - 5 : i + 1]) - min(prices[i - 5 : i + 1])
            ...
            logger.warning(
                "Error calculating technical indicators", symbol=symbol, error=str(e)
            )
    ...
    def generate_technical_indicators(
        self, symbol: str, market_data: MockMarketData
        ...
        return MockTechnicalIndicators(
            ...
            macd_histogram=basic_indicators.get("macd", 0.0)
            - basic_indicators.get("macd_signal", 0.0),
    ...
    def generate_markov_analysis(
        self, symbol: str, current_data: MockMarketData, regime: MarketRegime
    ) -> MockMarkovAnalysis:
    ...
    async def stream_market_data(
        ...
        for i, data_point in enumerate(scenario_data):
            if i % (interval_seconds // 60) == 0:  # Adjust for interval
                ...
                markov_analysis = self.generate_markov_analysis(
                    symbol, data_point, regime
                )
    ...
    def save_scenario_data(
        self,
        scenarios: Dict[str, List[MockMarketData]],
    ...
    def load_scenario_data(
        self, file_path: str = "mock_scenarios.json"
    ) -> Dict[str, List[MockMarketData]]:
        ...
        try:
            ...
            for scenario_name, data_points in serializable_scenarios.items():
                ...
                for point_dict in data_points:
                    ...
                    point_dict["timestamp"] = datetime.fromisoformat(
