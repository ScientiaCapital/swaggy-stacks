...
from app.analysis.tool_feedback_tracker import (
    ToolFeedbackTracker,
    tool_feedback_tracker,
)
...
class AgentTestingFramework:
    ...
    def __init__(
        ...
        self.agent_coordinator = agent_coordinator or AIAgentCoordinator(
    ...
    def create_test_scenarios(self) -> Dict[str, List[AgentDecisionTest]]:
        ...
        for regime in MarketRegime:
            ...
            for symbol in test_symbols:
                ...
                for i, market_data in enumerate(
                    market_data_points[::3]
                ):  # Every 3rd point
                    # Generate technical indicators
                    tech_indicators = self.mock_generator.generate_technical_indicators(
                        symbol, market_data
                    )
                    ...
                    expected_decisions = self._get_expected_decisions_for_regime(
                        regime, tech_indicators.to_dict()
                    )
    ...
    def _get_expected_decisions_for_regime(
        self, regime: MarketRegime, tech_indicators: Dict[str, Any]
    ) -> List[str]:
    ...
    async def run_single_agent_test(
        self, test: AgentDecisionTest, agent_type: str
    ) -> AgentTestResult:
        ...
        try:
            ...
            # Mock account info
            account_info = {"equity": 100000, "buying_power": 100000, "cash": 50000}
    ...
    def _extract_reasoning_from_comprehensive(self, result: Dict[str, Any]) -> str:
        ...
        if "strategy_signal" in result and "reasoning" in result["strategy_signal"]:
            reasoning_parts.append(
                f"Strategy: {result['strategy_signal']['reasoning']}"
            )

        return (
            " | ".join(reasoning_parts) if reasoning_parts else "Comprehensive analysis"
        )
    ...
    async def run_comprehensive_agent_test(
        ...
        if regime_filter:
            test_scenarios = {
                k: v for k, v in self.test_scenarios.items() if k in regime_filter
            }
        ...
        for agent_type in agent_types:
            ...
            for regime, tests in test_scenarios.items():
                ...
                for i in range(0, len(tests), batch_size):
                    ...
                    tasks = [
                        self.run_single_agent_test(test, agent_type) for test in batch
                    ]
                    ...
                    for result in batch_results:
                        if isinstance(result, AgentTestResult):
                            ...
                            logger.error(
                                "Test batch execution failed", error=str(result)
                            )
    ...
    def _generate_performance_report(
        self, agent_type: str, test_results: List[AgentTestResult]
        ...
        passed_tests = sum(
            1 for result in test_results if result.result == TestResult.PASSED
        )
        failed_tests = sum(
            1 for result in test_results if result.result == TestResult.FAILED
        )
        error_tests = sum(
            1 for result in test_results if result.result == TestResult.ERROR
        )
        ...
        average_confidence = (
            statistics.mean([r.confidence for r in valid_results])
            if valid_results
            else 0.0
        )
        average_response_time_ms = statistics.mean(
            [r.response_time_ms for r in test_results]
        )
        ...
        for regime, regime_results in regime_groups.items():
            ...
            decision_accuracy_by_regime[regime] = correct_decisions / len(
                regime_results
            )
        ...
        for result in test_results:
            if result.result == TestResult.FAILED:
                ...
                if not result.decision_correct:
                    reasons.append(
                        f"Wrong decision: got {result.actual_decision}, expected {result.expected_decisions}"
                    )
        ...
        common_failures = [
            f"{reason} ({count} times)" for reason, count in common_failures
        ]
        ...
        if average_confidence < 0.6:
            recommendations.append(
                f"Average confidence is low ({average_confidence:.1%})"
            )
        if average_response_time_ms > 3000:
            recommendations.append(
                f"Average response time is high ({average_response_time_ms:.0f}ms)"
            )
        ...
        for regime, accuracy in decision_accuracy_by_regime.items():
            if accuracy < 0.7:
                recommendations.append(
                    f"Poor performance in {regime} regime ({accuracy:.1%} accuracy)"
                )
    ...
    def generate_test_report(self) -> Dict[str, Any]:
        ...
        # Overall statistics
        passed_tests = sum(
            1 for result in self.test_results if result.result == TestResult.PASSED
        )
        failed_tests = sum(
            1 for result in self.test_results if result.result == TestResult.FAILED
        )
        error_tests = sum(
            1 for result in self.test_results if result.result == TestResult.ERROR
        )
        ...
        report = {
            "executive_summary": {
                ...
                "regimes_tested": len(
                    set(r.metadata.get("regime") for r in self.test_results)
                ),
            },
            ...
            "test_execution_details": {
                ...
                "test_results": [
                    asdict(result) for result in self.test_results[-50:]
                ],  # Last 50 results
            },
    ...
    def _generate_overall_recommendations(self) -> List[str]:
        ...
        avg_success_rate = statistics.mean(
            report.success_rate for report in self.performance_reports.values()
        )

        if avg_success_rate < 0.7:
            recommendations.append(
                "Overall agent performance is below acceptable threshold (70%)"
            )
        elif avg_success_rate > 0.9:
            recommendations.append(
                "Excellent overall agent performance - consider more challenging test scenarios"
            )

        # Agent comparison
        agent_success_rates = {
            agent: report.success_rate
            for agent, report in self.performance_reports.items()
        }
        ...
        if len(agent_success_rates) > 1:
            recommendations.append(
                f"Best performing agent: {best_agent[0]} ({best_agent[1]:.1%} success rate)"
            )
            recommendations.append(
                f"Worst performing agent: {worst_agent[0]} ({worst_agent[1]:.1%} success rate)"
            )
    ...
    async def run_stress_test(
        ...
        successful_requests = sum(
            1 for r in stress_test_results if r.result == TestResult.PASSED
        )
        ...
        stress_report = {
            ...
            "performance_assessment": (
                "healthy"
                if successful_requests / total_requests > 0.9
