...
class AITradingAdvisor:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if await self._orchestrator.is_server_available(MCPServerType.ANTHROPIC_CLAUDE):
                available_servers.append("Anthropic Claude")
            
            if not available_servers:
                logger.warning("No AI servers available - advisor will have limited functionality")
            else:
                logger.info(f"AI Trading Advisor initialized with servers: {', '.join(available_servers)}")
    ...
    async def analyze_market(self, request: AIAnalysisRequest) -> AIAnalysisResult:
        ...
        try:
            ...
            result = await self._parse_ai_response(
                response, request, model, start_time
            )
            
            # Update performance metrics
            self._update_metrics(model, True, (datetime.now() - start_time).total_seconds())
            ...
            self._update_metrics(model, False, (datetime.now() - start_time).total_seconds())
            logger.error(
                "AI analysis failed",
                symbol=request.symbol,
                analysis_type=request.analysis_type.value,
    ...
    def _build_analysis_prompt(self, request: AIAnalysisRequest) -> str:
        ...
        return base_prompts.get(request.analysis_type, "Provide market analysis for the given data.")
    ...
    async def _call_ai_model(
        ...
        try:
            if model in [AIModel.GPT_4, AIModel.GPT_4_TURBO]:
                ...
                response = await self._orchestrator.call_openai_gpt("chat_completion", **payload)
            else:
                # Call Anthropic Claude server
                response = await self._orchestrator.call_anthropic_claude("messages", **payload)
            ...
            if fallback_model and fallback_model != model:
                logger.warning(f"Falling back from {model.value} to {fallback_model.value}")
    ...
    def _extract_confidence(self, text: str) -> float:
        ...
        if "high confidence" in text_lower or "very confident" in text_lower:
            return 0.9
        elif "moderate confidence" in text_lower or "moderately confident" in text_lower:
            return 0.7
    ...
    def _extract_recommendations(self, text: str) -> List[str]:
        ...
        for line in lines:
            ...
            elif in_recommendation_section and line.startswith(('-', '•', '*', str(len(recommendations) + 1))):
                recommendations.append(line)
    ...
    def _determine_risk_level(self, text: str) -> str:
        ...
        high_risk_keywords = ['high risk', 'very risky', 'significant risk', 'dangerous', 'volatile']
        low_risk_keywords = ['low risk', 'safe', 'conservative', 'stable', 'minimal risk']
    ...
    def _update_metrics(self, model: AIModel, success: bool, response_time: float):
        ...
        current_total_time = self.avg_response_times[model] * (self.request_counts[model] - 1)
        self.avg_response_times[model] = (current_total_time + response_time) / self.request_counts[model]
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "server_status": {
                "openai_available": await self._orchestrator.is_server_available(MCPServerType.OPENAI_GPT) if self._orchestrator else False,
                "anthropic_available": await self._orchestrator.is_server_available(MCPServerType.ANTHROPIC_CLAUDE) if self._orchestrator else False
            },
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._orchestrator:
                ...
                if await self._orchestrator.is_server_available(MCPServerType.ANTHROPIC_CLAUDE):
                    health_data["available_models"].extend(["claude-3-5-sonnet", "claude-3-opus"])
