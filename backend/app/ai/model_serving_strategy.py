...
class MemoryEfficientServingStrategy:
    ...
    def __init__(
        ...
        logger.info(
            f"🧠 Initializing serving strategy: {deployment_mode.value}, {available_memory_gb}GB"
        )
    ...
    def select_optimal_models(self) -> List[ModelConfig]:
        ...
        available_memory = (
            self.available_memory_gb * 0.8
        )  # 80% utilization safety margin
        selected = []
        total_memory = 0.0

        # Sort by priority and memory efficiency
        candidates = sorted(
            self.models.values(), key=lambda m: (m.priority, m.memory_requirement_gb)
        )

        for model in candidates:
            if (
                self.deployment_mode == DeploymentMode.CONTAINER_CPU
                and not model.api_fallback
            ):
            ...
            if total_memory + model.memory_requirement_gb <= available_memory:
                ...
                logger.info(
                    f"✅ Selected {model.name}: {model.memory_requirement_gb}GB ({model.specialization})"
                )
            else:
                if model.api_fallback:
                    ...
                    logger.warning(
                        f"❌ Skipped {model.name}: Insufficient memory ({model.memory_requirement_gb}GB)"
                    )

        self.selected_models = selected
        logger.info(
            f"📊 Total local memory usage: {total_memory:.1f}GB / {available_memory:.1f}GB"
        )
        return selected
    ...
    def generate_deployment_config(self) -> Dict:
        ...
        for model in self.selected_models:
            ...
            if (
                ...
                in [DeploymentMode.NATIVE_MLX, DeploymentMode.HYBRID]
                and model.memory_requirement_gb <= self.available_memory_gb
            ):
        ...
        config["fallback_strategy"] = {
            ...
            "retry_attempts": 2,
            "fallback_order": [
                m.name for m in sorted(self.selected_models, key=lambda x: x.priority)
            ],
        }
    ...
    def estimate_inference_performance(self) -> Dict:
        ...
        for model in self.selected_models:
            ...
            if (
                model.mlx_optimized
                and self.deployment_mode != DeploymentMode.CONTAINER_CPU
            ):
...
def create_serving_strategy(memory_gb: float = 8.0, mode: str = "hybrid") -> Dict:
    ...
    return {
        ...
        "total_local_models": len(
            [
                m
                for m in strategy.selected_models
                if m.memory_requirement_gb <= memory_gb
            ]
        ),
        "total_api_models": len(
            [m for m in strategy.selected_models if m.memory_requirement_gb > memory_gb]
        ),
    }
