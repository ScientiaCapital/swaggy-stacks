...
from app.core.exceptions import MCPError, MCPConnectionError, MCPTimeoutError
...
class MCPOrchestrator:
    ...
    async def initialize(self, config_overrides: Optional[Dict] = None):
        ...
        async with self._lock:
            try:
                ...
                logger.info("MCPOrchestrator fully initialized",
                          enabled_servers=[s.name for s in self._server_configs.values() if s.enabled],
    ...
    async def _apply_config_overrides(self, overrides: Dict):
        ...
        for server_name, config in overrides.items():
            ...
            for st in MCPServerType:
                if st.value == server_name or self._server_configs.get(st, {}).name == server_name:
                    server_type = st
                    break
        ...
        for server_type, config in self._server_configs.items():
            if config.enabled:
                connection_tasks.append(
                    self._connect_to_server(server_type)
                )
    ...
    async def get_server_status(self, server_type: Optional[MCPServerType] = None) -> Union[MCPServerStatus, Dict[MCPServerType, MCPServerStatus]]:
    ...
    async def call_sequential_thinking(self, method: str, **kwargs) -> Any:
        ...
        return await self._call_server(MCPServerType.SEQUENTIAL_THINKING, method, **kwargs)
    ...
    async def call_shrimp_task_manager(self, method: str, **kwargs) -> Any:
        ...
        return await self._call_server(MCPServerType.SHRIMP_TASK_MANAGER, method, **kwargs)
    
    async def _call_server(self, server_type: MCPServerType, method: str, **kwargs) -> Any:
        ...
        try:
            ...
            raise MCPTimeoutError(f"Call to {server_type.value} timed out after {config.timeout}s")
        except Exception as e:
