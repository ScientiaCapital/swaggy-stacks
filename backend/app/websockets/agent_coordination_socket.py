...
class AgentDecisionUpdate:
    ...
    agent_id: str
    agent_type: str  # market_analyst, risk_advisor, strategy_optimizer, performance_coach
    symbol: str
...
class AgentCoordinationManager:
    ...
    async def connect(self, websocket: WebSocket, client_type: str = "dashboard"):
        ...
        logger.info("Agent coordination WebSocket established", 
    ...
    def disconnect(self, websocket: WebSocket):
        ...
        logger.info("Agent coordination WebSocket disconnected",
                   connections=len(self.active_connections))
    
    async def subscribe_to_agent_decisions(self, websocket: WebSocket, agent_ids: List[str]):
    ...
    async def subscribe_to_coordination_channel(self, websocket: WebSocket, channel: str):
    ...
    async def broadcast_agent_decision(self, decision: AgentDecisionUpdate):
        ...
        await self._broadcast_to_agent_subscribers("agent_decisions", decision.agent_id, message)
    ...
    async def broadcast_tool_execution_result(self, result: ToolExecutionResult):
        ...
        await self._broadcast_to_agent_subscribers("tool_feedback", result.agent_id, message)
    ...
    async def broadcast_agent_status(self, status: AgentStatusUpdate):
        ...
        await self._broadcast_to_agent_subscribers("status_updates", status.agent_type, message)
    
    async def send_direct_message(self, websocket: WebSocket, message_type: str, data: Any):
    ...
    async def _broadcast_to_agent_subscribers(self, subscription_type: str, identifier: str, message: dict):
        """Broadcast to clients subscribed to specific agent events"""
        disconnected = []
        
        for websocket in self.active_connections.copy():
            if websocket in self.agent_subscriptions:
                subscribed_items = self.agent_subscriptions[websocket].get(subscription_type, set())
                if identifier in subscribed_items or "all" in subscribed_items:
                    try:
                        await websocket.send_text(json.dumps(message))
                    except Exception as e:
                        logger.warning("Failed to broadcast agent message", error=str(e))
    ...
    async def _broadcast_to_coordination_channel(self, channel: str, message: dict):
        ...
        for websocket in self.coordination_channels[channel].copy():
            try:
                ...
                logger.warning("Failed to broadcast coordination message", 
    ...
    def get_active_agents(self) -> List[AgentStatusUpdate]:
        ...
        return [status for status in self.agent_status_cache.values() 
