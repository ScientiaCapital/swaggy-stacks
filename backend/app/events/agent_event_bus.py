...
class AgentEvent:
    ...
    def to_trading_message(self, source_service: str = "agent_event_bus") -> TradingMessage:
        ...
        return TradingMessage(
            ...
            correlation_id=self.correlation_id or f"agent_{self.agent_id}_{datetime.now().timestamp()}"
        )
...
class AgentEventBus:
    ...
    async def _setup_agent_queues(self):
        ...
        for queue_name, routing_patterns in agent_queues.items():
            ...
            logger.info(f"Set up agent queue: {queue_name} with patterns: {routing_patterns}")
            ...
            if agent_event.event_type in self.event_handlers:
                for handler in self.event_handlers[agent_event.event_type]:
                    try:
                        ...
                        logger.error("Agent event handler failed", 
    ...
    async def _broadcast_to_websocket(self, event: AgentEvent):
        ...
        try:
            if event.event_type == AgentEventType.DECISION_RESPONSE.value:
                ...
                await agent_coordination_manager.broadcast_agent_decision(decision_update)
                ...
                await agent_coordination_manager.broadcast_tool_execution_result(tool_result)
                
            elif event.event_type in [AgentEventType.COORDINATION.value, AgentEventType.CONSENSUS_REQUEST.value]:
                ...
                await agent_coordination_manager.broadcast_coordination_message(coord_message)
    ...
    async def publish_event(self, event: AgentEvent) -> bool:
        ...
        try:
            ...
            if success:
                logger.debug("Published agent event", 
    ...
    async def publish_market_update(self, agent_id: str, symbol: str, market_data: Dict[str, Any]) -> bool:
    ...
    async def publish_decision_request(self, agent_id: str, symbol: str, 
    ...
    async def publish_decision_response(self, agent_id: str, agent_type: str, symbol: str,
    ...
    async def publish_tool_execution(self, agent_id: str, tool_name: str,
    ...
    async def publish_coordination_message(self, sender_agent_id: str, recipient_agent_id: Optional[str],
    ...
    async def publish_status_update(self, agent_id: str, agent_type: str, status: str,
        ...
        event = AgentEvent(
            ...
            payload={
                ...
                "last_decision_time": datetime.now().isoformat() if status == "active" else None,
                "performance_metrics": performance_metrics or {}
            }
        )
        return await self.publish_event(event)
    
    async def request_consensus(self, requester_agent_id: str, symbol: str,
    ...
    def subscribe_to_event(self, event_type: str, handler: Callable[[AgentEvent], None]):
    ...
    def unsubscribe_from_event(self, event_type: str, handler: Callable[[AgentEvent], None]):
    ...
    async def get_agent_decision_history(self, agent_id: str, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    async def get_active_agents(self) -> List[str]:
        ...
        return [agent_id for agent_id, status in self.active_agents.items() 
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            "status": "healthy" if rabbitmq_health.get("status") == "healthy" else "degraded",
