...
class MultiAgentCoordinator:
    ...
    async def initialize(self):
        ...
        self.event_bus.subscribe_to_event(
            AgentEventType.CONSENSUS_RESPONSE.value, self._handle_consensus_response
        )

        self.event_bus.subscribe_to_event(
            AgentEventType.COORDINATION.value, self._handle_coordination_message
        )
    ...
    async def _handle_consensus_response(self, event: AgentEvent):
        ...
        if not consensus_id or consensus_id not in self.active_consensus:
            logger.warning(
                "Received consensus response for unknown request",
                consensus_id=consensus_id,
            )
            return
    ...
    async def _calculate_consensus(
        self, request: ConsensusRequest, votes: List[AgentVote], timeout: bool
    ) -> ConsensusResult:
        ...
        try:
            if request.consensus_method == ConsensusMethod.SIMPLE_MAJORITY:
                ...
                winning_votes = [
                    vote for vote in votes if vote.decision == final_decision
                ]
                confidence = statistics.mean(
                    [vote.confidence for vote in winning_votes]
                )
                ...
                if decision_scores:
                    ...
                    confidence = (
                        decision_scores[final_decision]
                        / decision_weights[final_decision]
                    )
                    ...
                    consensus_achieved = (
                        decision_weights[final_decision] > total_weight * 0.5
                    )
                ...
                if len(decision_counts) == 1:
                    ...
                    final_decision, confidence, conflict_resolution_applied = (
                        await self._resolve_conflict(request.conflict_resolution, votes)
                    )
                ...
                if decision_scores:
                    ...
                    total_weight = sum(
                        2.0 if vote.agent_type == "risk_advisor" else 1.0
            ...
            if has_conflict and request.consensus_method not in [
    ...
    async def _resolve_conflict(
        self, resolution_method: ConflictResolution, votes: List[AgentVote]
    ) -> Tuple[str, float, str]:
        ...
        if resolution_method == ConflictResolution.HIGHEST_CONFIDENCE:
            ...
            return (
                highest_confidence_vote.decision,
                highest_confidence_vote.confidence,
                "highest_confidence",
            )
            ...
            conservative_votes = sorted(
                votes, key=lambda v: decision_priority.get(v.decision, 0), reverse=True
            )
            best_vote = conservative_votes[0]
            return (
                best_vote.decision,
                best_vote.confidence * 0.8,
                "conservative_bias",
            ...
            if risk_votes:
                ...
                return (
                    highest_confidence_vote.decision,
                    highest_confidence_vote.confidence,
                    "risk_manager_override_fallback",
                )
            ...
            if total_weight > 0:
                ...
                confidence = (
                    statistics.mean([vote.confidence for vote in votes]) * 0.9
                )  # Reduce due to conflict
    ...
    def _update_agent_performance(
        self, votes: List[AgentVote], result: ConsensusResult
    ):
        """Update agent performance metrics"""
        ...
        for vote in votes:
            ...
            new_confidence = (
                old_confidence * (total_votes - 1) + vote.confidence
            ) / total_votes
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "consensus_success_rate": (
                sum(1 for r in self.consensus_history[-20:] if r.consensus_achieved)
                / min(20, len(self.consensus_history))
                if self.consensus_history
                else 0.0
            ),
            "timestamp": datetime.now().isoformat(),
