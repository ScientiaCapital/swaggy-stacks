...
class RiskTool:
    ...
    def _calculate_position_size(self, params: str) -> str:
        ...
        try:
            ...
            risk_amount = float(parts[3]) if len(parts) > 3 else 1000  # Default $1000 risk
            ...
            return json.dumps({
                ...
                "position_sizing": {
                    ...
                    "percent_risk": {
                        ...
                        "position_value": round(percent_risk_shares * entry_price, 2)
                    }
                },
                "recommended_size": int(min(fixed_risk_shares, kelly_shares, percent_risk_shares)),
                "risk_reward_ratio": round(abs((entry_price + (entry_price - stop_loss) * 1.5) - entry_price) / risk_per_share, 2)
            })
    ...
    def _assess_portfolio_risk(self, _: str = "") -> str:
        ...
        try:
            ...
            exposure_percentage = (invested_amount / total_value) * 100 if total_value > 0 else 0
    ...
    def _check_risk_limits(self, params: str) -> str:
        ...
        try:
            ...
            checks = {
                ...
                "cash_availability": {
                    "passed": position_value <= available_cash if action == "BUY" else True,
            ...
            return json.dumps({
                ...
                "approval": {
                    ...
                    "warnings": [
                        f"Position size exceeds limit" if not checks["position_size_check"]["passed"] else None,
                        f"Insufficient cash" if not checks["cash_availability"]["passed"] else None
                    ]
                }
            })
        ...
        try:
                ...
                if symbol1 in correlation_matrix.index:
                    for j, symbol2 in enumerate(symbols):
                        if symbol2 in correlation_matrix.columns and i != j:
                            ...
                            correlations[pair] = round(correlation_matrix.loc[symbol1, symbol2], 3)
            ...
            return json.dumps({
                ...
                "risk_analysis": {
                    ...
                    "interpretation": "Low portfolio diversification" if len(high_correlations) > 0 else "Good diversification"
                }
            })
        ...
        try:
            ...
            return json.dumps({
                ...
                "drawdown_analysis": {
                    ...
                    "recovery_status": "recovered" if recovery_idx else "still_in_drawdown",
                ...
                "interpretation": {
                    "risk_level": "high" if abs(max_drawdown) > 0.2 else "medium" if abs(max_drawdown) > 0.1 else "low",
                    "recovery_capability": "strong" if recovery_idx else "concerning"
                }
            })
