...
class AnalysisTool:
    ...
    def _technical_summary(self, symbol: str) -> str:
        ...
        try:
            ...
            # Moving averages
            ma20 = closes.rolling(20).mean().iloc[-1] if len(closes) >= 20 else current_price
            ma50 = closes.rolling(50).mean().iloc[-1] if len(closes) >= 50 else current_price
            ma200 = closes.rolling(200).mean().iloc[-1] if len(closes) >= 200 else current_price
            ...
            return json.dumps({
                ...
                "technical_indicators": {
                    "moving_averages": {
                        ...
                        "ma_alignment": self._assess_ma_alignment(current_price, ma20, ma50, ma200)
                    },
                    "momentum": {
                    ...
                    "volatility": {
                        ...
                        "volatility_rank": "high" if volatility > 30 else "medium" if volatility > 15 else "low"
                    }
                },
                "price_analysis": {
                    ...
                    "distance_from_high": round((year_high - current_price) / current_price * 100, 1),
                    "distance_from_low": round((current_price - year_low) / current_price * 100, 1)
                },
                "volume_analysis": {
                    "average_volume": int(avg_volume),
                    "recent_volume": int(recent_volume),
                    "volume_ratio": round(volume_ratio, 2),
                    "volume_trend": "above_average" if volume_ratio > 1.2 else "below_average" if volume_ratio < 0.8 else "normal"
                },
                "trend_analysis": trend_strength,
                "overall_signal": self._generate_overall_signal(current_rsi, trend_strength, current_price, ma20, ma50)
            })
        ...
        try:
            ...
            return json.dumps({
                ...
                "valuation_metrics": {
                    ...
                    "valuation_assessment": self._assess_valuation(pe_ratio, pb_ratio)
                },
                "profitability_metrics": {
                    ...
                    "profitability_score": self._score_profitability(roe, profit_margins, operating_margins)
                },
                "growth_metrics": {
                ...
                "dividend_metrics": {
                    ...
                    "dividend_sustainability": self._assess_dividend_sustainability(dividend_yield, payout_ratio)
                },
                "financial_health": {
                    "debt_to_equity": round(debt_to_equity, 2) if debt_to_equity else None,
                    "financial_strength": self._assess_financial_strength(debt_to_equity, roe)
                }
            })
    ...
    def _market_regime_detection(self, symbol: str = "SPY") -> str:
        ...
        try:
            ...
            vol_regime = "high" if recent_vol > historical_vol * 1.2 else "low" if recent_vol < historical_vol * 0.8 else "normal"
            
            # Volume regime
            recent_volume = volumes.tail(20).mean()
            historical_volume = volumes.mean()
            volume_regime = "high" if recent_volume > historical_volume * 1.2 else "low" if recent_volume < historical_volume * 0.8 else "normal"
            
            # Overall regime assessment
            regime_signals = [short_trend["regime"], medium_trend["regime"], long_trend["regime"]]
            dominant_regime = max(set(regime_signals), key=regime_signals.count)
            
            # Regime strength
            regime_strength = regime_signals.count(dominant_regime) / len(regime_signals)
            ...
            return json.dumps({
                ...
                "trading_implications": {
                    "strategy_recommendation": self._get_regime_strategy(dominant_regime, vol_regime),
                    "risk_level": "high" if vol_regime == "high" else "medium" if vol_regime == "normal" else "low",
                    "position_sizing": "reduced" if vol_regime == "high" else "normal"
                }
            })
    ...
    def _correlation_analysis(self, symbols_str: str) -> str:
        ...
        try:
            ...
            return json.dumps({
                ...
                "correlation_summary": {
                    "average_correlation": round(np.mean(list(correlations.values())), 3),
                    "highest_correlation": max(correlations.values()) if correlations else 0,
                    "lowest_correlation": min(correlations.values()) if correlations else 0,
                    "high_correlations": [(s1, s2, round(c, 3)) for s1, s2, c in high_correlations],
                    "low_correlations": [(s1, s2, round(c, 3)) for s1, s2, c in low_correlations]
                },
                "diversification_analysis": {
                    "diversification_benefit": len(high_correlations) == 0,
                    "concentration_risk": len(high_correlations) > len(symbols) // 2,
                    "recommendation": "good_diversification" if len(high_correlations) == 0 else "consider_rebalancing"
                }
            })
    ...
    def _sector_analysis(self, _: str = "") -> str:
        ...
        try:
            ...
            for sector, etf in sector_etfs.items():
                try:
                    ...
                    if not hist.empty:
                        ...
                        recent_vol = hist['Close'].pct_change().std() * np.sqrt(252) * 100
                        
                        sector_performance[sector] = {
                            "etf": etf,
                            "returns_3m": round(returns_3m, 2),
                            "volatility": round(recent_vol, 1),
                            "risk_adjusted_return": round(returns_3m / recent_vol if recent_vol > 0 else 0, 2)
                        }
                except:
                    continue
            ...
            # Rank sectors
            best_performers = sorted(sector_performance.items(), key=lambda x: x[1]["returns_3m"], reverse=True)[:3]
            worst_performers = sorted(sector_performance.items(), key=lambda x: x[1]["returns_3m"])[:3]
            
            # Risk-adjusted performance
            risk_adjusted = sorted(sector_performance.items(), key=lambda x: x[1]["risk_adjusted_return"], reverse=True)[:3]
            
            return json.dumps({
                "sector_performance": sector_performance,
                "sector_rankings": {
                    "best_performers_3m": [(sector, data["returns_3m"]) for sector, data in best_performers],
                    "worst_performers_3m": [(sector, data["returns_3m"]) for sector, data in worst_performers],
                    "best_risk_adjusted": [(sector, data["risk_adjusted_return"]) for sector, data in risk_adjusted]
                },
                "rotation_analysis": {
    ...
    def _volatility_analysis(self, symbol: str) -> str:
        ...
        try:
            ...
            return json.dumps({
                ...
                "volatility_analysis": {
                    "volatility_trend": "increasing" if vol_1m > vol_3m else "decreasing",
                    "volatility_rank": "high" if vol_1m > 30 else "medium" if vol_1m > 15 else "low",
                    "volatility_percentile": round(current_vol_percentile, 1),
                    "relative_volatility": "above_average" if vol_1m > vol_1y else "below_average"
                },
                "volatility_patterns": {
                ...
                "trading_implications": {
                    "option_premium": "expensive" if current_vol_percentile > 70 else "cheap" if current_vol_percentile < 30 else "fair",
                    "position_sizing": "reduce" if vol_1m > 40 else "increase" if vol_1m < 10 else "normal",
                    "strategy_recommendation": self._get_volatility_strategy(vol_1m, current_vol_percentile)
                }
            })
    ...
    def _momentum_analysis(self, symbol: str) -> str:
        ...
        try:
            ...
            returns = {
                "1_week": (current_price / closes.iloc[-5] - 1) * 100 if len(closes) >= 5 else 0,
                "1_month": (current_price / closes.iloc[-21] - 1) * 100 if len(closes) >= 21 else 0,
                "3_month": (current_price / closes.iloc[-63] - 1) * 100 if len(closes) >= 63 else 0,
                "6_month": (current_price / closes.iloc[-126] - 1) * 100 if len(closes) >= 126 else 0,
                "1_year": (current_price / closes.iloc[0] - 1) * 100 if len(closes) >= 252 else 0
            }
            ...
            return json.dumps({
                ...
                "momentum_summary": {
                    "overall_direction": "bullish" if momentum_consistency > 0.6 else "bearish" if momentum_consistency < 0.4 else "mixed",
                    "consistency_score": round(momentum_consistency, 2),
                    "strongest_timeframe": max(returns.items(), key=lambda x: abs(x[1]))[0],
                    "acceleration": {
                        "short_term": "accelerating" if roc_short > 2 else "decelerating" if roc_short < -2 else "stable",
                        "long_term": "accelerating" if roc_long > 5 else "decelerating" if roc_long < -5 else "stable"
                    }
                },
                "trading_signals": {
                    "trend_following": "buy" if momentum_consistency > 0.7 else "sell" if momentum_consistency < 0.3 else "hold",
                    "mean_reversion": "sell" if returns["1_week"] > 10 else "buy" if returns["1_week"] < -10 else "hold",
                    "momentum_quality": "high" if momentum_consistency > 0.8 and abs(returns["3_month"]) > 10 else "low"
                }
            })
    ...
    def _comparative_analysis(self, symbols_str: str) -> str:
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    comparison_data[symbol] = {
                        "current_price": round(current_price, 2),
                        "returns": {
                            "1_month": round((current_price / closes.iloc[-21] - 1) * 100, 2) if len(closes) >= 21 else 0,
                            "3_month": round((current_price / closes.iloc[-63] - 1) * 100, 2) if len(closes) >= 63 else 0,
                            "6_month": round((current_price / closes.iloc[0] - 1) * 100, 2)
                        },
                        "volatility": round(returns.std() * np.sqrt(252) * 100, 1),
                        "sharpe_ratio": round((returns.mean() * 252) / (returns.std() * np.sqrt(252)), 2),
            ...
            # Rankings
            rankings = {
                "best_1m_return": sorted(comparison_data.items(), key=lambda x: x[1]["returns"]["1_month"], reverse=True),
                "best_6m_return": sorted(comparison_data.items(), key=lambda x: x[1]["returns"]["6_month"], reverse=True),
                "best_sharpe": sorted(comparison_data.items(), key=lambda x: x[1]["sharpe_ratio"], reverse=True),
                "lowest_volatility": sorted(comparison_data.items(), key=lambda x: x[1]["volatility"]),
                "smallest_drawdown": sorted(comparison_data.items(), key=lambda x: x[1]["max_drawdown"])
            }
            ...
            return json.dumps({
                ...
                "investment_recommendation": {
                    ...
                    "value_pick": min(comparison_data.items(), key=lambda x: x[1]["pe_ratio"] if x[1]["pe_ratio"] > 0 else 999)[0],
    ...
    def _assess_ma_alignment(self, price: float, ma20: float, ma50: float, ma200: float) -> str:
    ...
    def _generate_overall_signal(self, rsi: float, trend: dict, price: float, ma20: float, ma50: float) -> dict:
        ...
        if buy_signals > sell_signals:
            return {"signal": "BUY", "strength": buy_signals, "confidence": buy_signals / len(signals)}
        elif sell_signals > buy_signals:
            return {"signal": "SELL", "strength": sell_signals, "confidence": sell_signals / len(signals)}
        else:
    ...
    def _score_profitability(self, roe: float, profit_margin: float, operating_margin: float) -> str:
        ...
        return "excellent" if score == 3 else "good" if score == 2 else "average" if score == 1 else "poor"
    ...
    def _assess_dividend_sustainability(self, dividend_yield: float, payout_ratio: float) -> str:
    ...
    def _analyze_market_style(self, sector_performance: dict) -> str:
        ...
        growth_avg = np.mean([
            sector_performance[s]["returns_3m"] for s in growth_sectors 
            if s in sector_performance
        ]) if any(s in sector_performance for s in growth_sectors) else 0
        
        value_avg = np.mean([
            sector_performance[s]["returns_3m"] for s in value_sectors 
            if s in sector_performance
        ]) if any(s in sector_performance for s in value_sectors) else 0
        
        defensive_avg = np.mean([
            sector_performance[s]["returns_3m"] for s in defensive_sectors 
            if s in sector_performance
        ]) if any(s in sector_performance for s in defensive_sectors) else 0
    ...
    def _detect_volatility_clustering(self, returns: pd.Series) -> dict:
        ...
        if len(returns) < 50:
            return {"has_clustering": False, "regime_changes": 0, "stability_score": 0.5}
        ...
        regime_changes = ((rolling_vol > vol_mean + vol_std) | 
                         (rolling_vol < vol_mean - vol_std)).sum()
        
        # Stability score
        stability_score = 1 - (vol_changes.mean() / rolling_vol.mean()) if rolling_vol.mean() > 0 else 0.5
