...
class PatternTool:
            ...
            return json.dumps(
                {
                    ...
                    "trading_implications": self._get_pattern_implications(
                        patterns_found
                    ),
                }
            )
    ...
    def _detect_candlestick_patterns(self, symbol: str) -> str:
        ...
        try:
            ...
            for i in range(lookback):
                ...
                volume_conf = (
                    self._calculate_volume_confirmation(volumes, abs(idx))
                ...
                # Pin Bar patterns (Hammer/Shooting Star)
                    ...
                    lower_shadow > body_size * 2 and upper_shadow < body_size and c > o
                ):  # Hammer/Pin Bar
                    ...
                    upper_shadow > body_size * 2 and lower_shadow < body_size and o > c
                ...
                if i < lookback - 1:
                    ...
                    if (
                        c > o
                        and prev_c < prev_o  # Current bullish, prev bearish
                        and c > prev_o
                        and o < prev_c
                        ...
                        o > c
                        and prev_o < prev_c  # Current bearish, prev bullish
                        and o > prev_c
                        and c < prev_o
                    ):  # Current engulfs previous
                        confidence = 0.8 * volume_conf
                    ...
                    # Harami patterns (Inside Bar)
                        ...
                        body_size < prev_body_size * 0.8  # Current body smaller
                        and max(o, c) < max(prev_o, prev_c)  # Current highs lower
                    ...
                    if abs(h - prev_h) < tolerance and h == max(
                        h, prev_h
                    ):  # Tweezers Top
                    ...
                    elif abs(l - prev_l) < tolerance and l == min(
                        l, prev_l
                    ):  # Tweezers Bottom
                        confidence = 0.6 * volume_conf
                        if trend_context == "bearish":
                ...
                if i < lookback - 2:  # Need 3 candles
                    ...
                    if (
                        ...
                        and c2_body < c1_body * 0.3  # Middle candle small body
                        and c3_c > c3_o  # Third candle bullish
                        and c3_c
                        > (c1_o + c1_c) / 2  # Third closes above midpoint of first
                        and c2_h < c1_l
                    ):  # Gap down condition
                    ...
                    # Evening Star: Bullish -> Small body -> Bearish
                        ...
                        and c2_body < c1_body * 0.3  # Middle candle small body
                        and c3_c < c3_o  # Third candle bearish
                        and c3_c
                        < (c1_o + c1_c) / 2  # Third closes below midpoint of first
                        and c2_l > c1_h
                    ):  # Gap up condition
                        confidence = 0.85 * volume_conf
                        if trend_context == "bullish":
            ...
            return json.dumps(
                {
                    ...
                    "pattern_summary": {
                        ...
                        "bullish_signals": len(
                            [p for p in patterns if "bullish" in p["significance"]]
                        ),
                        "bearish_signals": len(
                            [p for p in patterns if "bearish" in p["significance"]]
                        ),
                        "indecision_signals": len(
                            [p for p in patterns if "indecision" in p["significance"]]
                        ),
                    },
            ...
            return json.dumps(
                {
                    ...
                    "trading_zone": {
                        "in_support_zone": any(
                            abs(current_price - s) / current_price < 0.02
                            for s in support_levels
                        ),
                        "in_resistance_zone": any(
                            abs(current_price - r) / current_price < 0.02
                            for r in resistance_levels
                        ),
                        "trend_bias": (
                            "bullish"
                            if nearest_support
                            and (current_price - nearest_support) / current_price < 0.05
                            else (
                                "bearish"
                                if nearest_resistance
                                and (nearest_resistance - current_price) / current_price
                                < 0.05
                                else "neutral"
                            )
                        ),
    ...
    def _analyze_volume_patterns(self, symbol: str) -> str:
        ...
        try:
            ...
            volume_trend = (
                ...
                / volumes.rolling(50).mean().iloc[-1]
                if len(volumes) >= 50
                else 1
            )
            ...
            volume_price_corr = (
                volumes.corr(price_changes.abs()) if len(volumes) > 10 else 0
            )
            ...
            spike_dates = (
                recent_spikes[recent_spikes].index.strftime("%Y-%m-%d").tolist()
            )
            ...
            return json.dumps(
                {
                    ...
                    "volume_analysis": {
                        ...
                        "volume_trend": (
                            "increasing"
                            if volume_trend > 1.1
                            else "decreasing" if volume_trend < 0.9 else "stable"
                        ),
                    },
                    "volume_patterns": {
                        ...
                        "volume_confirmation": volume_trend > 1
                        and volume_price_corr > 0.3,
                    },
                    "interpretation": {
                        "trend_strength": (
                            "strong"
                            if recent_volume > avg_volume * 1.5
                            else "moderate" if recent_volume > avg_volume else "weak"
                        ),
                        "institutional_activity": (
                            "high" if recent_volume > avg_volume * 2 else "normal"
                        ),
                        "breakout_potential": recent_volume > avg_volume * 2
                        and volume_price_corr > 0.5,
                    },
                }
            )
    ...
    def _detect_trend_patterns(self, symbol: str) -> str:
        ...
        try:
            ...
            long_trend = (
                self._calculate_trend(closes.tail(100))
                if len(closes) >= 100
                else medium_trend
            )
            ...
            volume_momentum = (
                hist["Volume"].pct_change(10).iloc[-1] if "Volume" in hist else 0
            )
            ...
            return json.dumps(
                {
                    ...
                    "trend_characteristics": {
                        ...
                        "momentum": (
                            "accelerating" if price_momentum > 0 else "decelerating"
                        ),
                        "volume_confirmation": volume_momentum > 0
                        and price_momentum > 0,
                    },
                    "reversal_signals": {
                        "trend_exhaustion": trend_strength > 3
                        and short_trend["r_squared"] < 0.7,
                        "momentum_divergence": (price_momentum > 0)
                        != (volume_momentum > 0),
            ...
            return json.dumps(
                {
                    ...
                    "breakout_probability": {
                        "upward_breakout": round(
                            (1 - resistance_distance)
                            * (1 if not volume_contraction else 1.2)
                            * (0.8 if price_volatility > 0.02 else 1.2),
                            2,
                        ),
    ...
    def _detect_head_shoulders(self, highs: np.ndarray, lows: np.ndarray) -> List[dict]:
        ...
        if len(peak_indices) >= 3:
            ...
            if (
                ...
                and abs(left_shoulder - right_shoulder) / head < 0.05
            ):
                patterns.append(
                    {
    ...
    def _find_peaks(self, data: np.ndarray, prominence: float) -> List[int]:
        ...
        for i in range(1, len(data) - 1):
            if (
                data[i] > data[i - 1]
                and data[i] > data[i + 1]
                and data[i] > np.mean(data) * (1 + prominence)
            ):
                peaks.append(i)
        return peaks
    ...
    def _calculate_trend(self, series: pd.Series) -> dict:
        ...
        try:
            ...
            return {
                ...
                "strength": (
                    "strong"
                    if abs(slope_percent) > 2
                    else "moderate" if abs(slope_percent) > 1 else "weak"
                ),
            }
        except Exception:
            return {"direction": "unknown", "slope_percent": 0, "r_squared": 0}
    ...
    def _calculate_reversal_probability(
        self, short: dict, medium: dict, strength: float
    ) -> float:
