...
class AgentRAGService:
    ...
    async def _handle_pattern_similarity(self, query: RAGQuery) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            embedding_result = await self.embedding_service.get_embedding(query.query_text)
            ...
            async with self._get_db_connection() as conn:
                ...
                """, query.agent_id, query_embedding.tolist(), 
                    query.similarity_threshold, query.max_results)
                ...
                for row in results:
                    ...
                    enriched_results.append({
                        ...
                        "pattern_metadata": pattern_meta['pattern_metadata'] if pattern_meta else {},
                        "market_data": pattern_meta['market_data'] if pattern_meta else {},
                        "last_seen": pattern_meta['last_seen'] if pattern_meta else None,
    ...
    async def _handle_decision_history(self, query: RAGQuery) -> List[Dict[str, Any]]:
        ...
        try:
            async with self._get_db_connection() as conn:
                ...
                time_filter = datetime.now() - timedelta(hours=query.context_window_hours)
        ...
        try:
            ...
            async with self._get_db_connection() as conn:
                ...
                results = await conn.fetch("""
                    ...
                    LIMIT $3
                """, query_embedding.tolist(), query.similarity_threshold, query.max_results)
    ...
    async def _handle_performance_analysis(self, query: RAGQuery) -> List[Dict[str, Any]]:
    ...
    async def _generate_recommendations(self, query: RAGQuery, results: List[Dict[str, Any]]) -> List[str]:
        """Generate contextual recommendations based on query results"""
        recommendations = []
        
        try:
            if not results:
                return ["No relevant patterns found - consider expanding search criteria"]
            
            # Analyze result patterns
            high_confidence_results = [r for r in results if r.get('confidence_score', 0) > 0.8]
            recent_results = [r for r in results if self._is_recent_result(r)]
            
            # Pattern-specific recommendations
            if query.query_type == QueryType.PATTERN_SIMILARITY:
                successful_patterns = [r for r in results if r.get('success_rate', 0) > 0.7]
                ...
                frequent_patterns = [r for r in results if r.get('occurrence_count', 0) > 10]
            ...
            if query.query_type == QueryType.DECISION_HISTORY:
                profitable_decisions = [
                    r for r in results 
                    if r.get('outcome_pnl', 0) > 0
                ]
                if profitable_decisions:
                    avg_profit = sum(r.get('outcome_pnl', 0) for r in profitable_decisions) / len(profitable_decisions)
                    recommendations.append(
    ...
    def _calculate_relevance_score(self, similarity: float, success_rate: float, occurrence_count: int) -> float:
    ...
    def _calculate_decision_relevance(self, decision: Dict[str, Any], query: RAGQuery) -> float:
    ...
    def _calculate_performance_score(self, performance: Dict[str, Any]) -> float:
        ...
        pnl_normalized = max(-1, min(1, performance.get('total_pnl', 0) / 10000))  # Normalize PnL
        sharpe = min(1, max(0, (performance.get('sharpe_ratio', 0) + 2) / 4))  # Normalize Sharpe
    ...
    def _calculate_avg_confidence(self, results: List[Dict[str, Any]]) -> float:
        ...
        for result in results:
            ...
            for key in ['confidence_score', 'similarity_score', 'relevance_score', 'confidence']:
    ...
    def _update_stats(self, duration_ms: int, results_count: int, recommendations_count: int):
        ...
        self._stats["avg_query_time_ms"] = (current_avg * (query_count - 1) + duration_ms) / query_count
