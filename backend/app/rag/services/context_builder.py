...
class TradingContextBuilder:
    ...
    async def create_initial_state(
        ...
        logger.info(f"Created initial trading state for {symbol} (session: {session_id})")
        return state
    ...
    async def add_memory_context(self, state: TradingState, query_text: str) -> TradingState:
        ...
        try:
            ...
            if memory_context:
                state["conversation_history"].append(
                    AIMessage(content=f"Found {len(memory_context)} similar historical patterns")
                )
    ...
    async def add_strategy_signal(self, state: TradingState, signal: TradingSignal) -> TradingState:
        ...
        # Add to conversation
        state["conversation_history"].append(
            AIMessage(content=f"{strategy_name} signal: {signal.action} ({signal.confidence:.1%} confidence)")
        )
    ...
    async def update_risk_context(self, state: TradingState) -> TradingState:
        ...
        try:
            ...
            state["risk_approval"] = await self._assess_risk_approval(risk_metrics, state)
    ...
    async def create_decision_context(self, state: TradingState) -> TradingState:
        ...
        try:
            ...
            if state["strategy_consensus"]:
                primary_strategy = max(state["strategy_consensus"], key=state["strategy_consensus"].get)
            ...
            if state["risk_metrics"].total_exposure > state["risk_metrics"].portfolio_value * 0.8:
                risk_factors.append("High portfolio exposure")
            
            # Opportunity factors
            opportunity_factors = []
            if state["market_data"].volatility and state["market_data"].volatility > 0.02:
            ...
            # Add to conversation
            state["conversation_history"].append(
                AIMessage(content=f"Decision context created - Primary strategy: {primary_strategy} ({avg_confidence:.1%} confidence)")
            )
    ...
    async def create_execution_plan(self, state: TradingState, action: str) -> TradingState:
        ...
        try:
            ...
            # Add to conversation
            state["conversation_history"].append(
                AIMessage(content=f"Execution plan created: {action} {max_quantity:.2f} shares of {state['symbol']}")
            )
    ...
    async def update_workflow_step(self, state: TradingState, current_step: str, next_steps: List[str] = None) -> TradingState:
    ...
    async def _build_risk_context(self) -> RiskMetrics:
        ...
        if not self.trading_manager:
            return RiskMetrics(portfolio_value=100000, available_cash=100000, total_exposure=0)
        ...
        try:
            ...
            if isinstance(portfolio_status, dict):
                ...
                available_cash = float(portfolio_status.get("available_cash", portfolio_value))
                ...
                return RiskMetrics(
                    ...
                    risk_score=min(total_exposure / portfolio_value, 1.0) if portfolio_value > 0 else 0.0
                )
            else:
                return RiskMetrics(portfolio_value=100000, available_cash=100000, total_exposure=0)
    ...
    async def _assess_risk_approval(self, risk_metrics: RiskMetrics, state: TradingState) -> bool:
        ...
        try:
            ...
            max_concentration = max(risk_metrics.position_concentration.values()) if risk_metrics.position_concentration else 0
