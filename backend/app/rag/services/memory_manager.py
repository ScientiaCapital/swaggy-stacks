...
class AgentMemoryManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            async with self._get_db_connection() as conn:
                ...
                if len(tables) < 3:
                    logger.warning(
                        "Some required tables are missing from database schema"
                    )
                else:
    ...
    async def store_memory(
        ...
        try:
            ...
            logger.debug(
                f"Stored {memory_type} memory for agent {agent_id}: ID {memory_id}"
            )
    ...
    async def _search_memories_by_type(
        self, memory_type: MemoryType, query: MemoryQuery, query_embedding: np.ndarray
    ) -> List[Memory]:
        ...
        async with self._get_db_connection() as conn:
            if memory_type == MemoryType.PATTERN:
                ...
                return await self._search_decision_memories(
                    conn, query, query_embedding
                )
            else:  # EPISODIC or KNOWLEDGE
                return await self._search_knowledge_memories(
                    conn, query, query_embedding, memory_type
                )
    ...
    async def _search_pattern_memories(
        self, conn: asyncpg.Connection, query: MemoryQuery, query_embedding: np.ndarray
    ) -> List[Memory]:
        ...
        for row in rows:
            metadata = (
                json.loads(row["pattern_metadata"]) if row["pattern_metadata"] else {}
            )
    ...
    async def _search_decision_memories(
        ...
        for row in rows:
            market_context = (
                json.loads(row["market_context"]) if row["market_context"] else {}
            )
            metadata = {
                "symbol": row["symbol"],
            ...
            memory = Memory(
                ...
                or f"Decision: {row['decision']} on {row['symbol']}",
                metadata=metadata,
    ...
    async def consolidate_memories(self, agent_id: str) -> int:
        ...
        try:
            ...
            if consolidated_count > 0:
                logger.info(
                    f"Consolidated {consolidated_count} memories for agent {agent_id}"
                )
    ...
    async def _consolidate_pattern_memories(self, agent_id: str) -> int:
        ...
        async with self._get_db_connection() as conn:
            ...
            similar_pairs = await conn.fetch(
                sql, agent_id, self.consolidation_threshold
            )
