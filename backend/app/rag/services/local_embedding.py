...
class LocalEmbeddingService:
    ...
    async def embed_texts(self, texts: List[str]) -> List[EmbeddingResult]:
        ...
        logger.debug(
            f"Processed {len(texts)} embeddings in {processing_time:.3f}s "
        ...
        for i in range(0, len(texts), self.batch_size):
            ...
            try:
                ...
                embeddings = await loop.run_in_executor(
                    self.executor, self._generate_batch_embeddings, batch
                )
    ...
    def _pad_to_superbpe_dims(self, embedding: np.ndarray) -> np.ndarray:
        ...
        if embedding.shape[0] > 0:
            ...
            if remainder > 0:
                weight = 0.05
                padded[current_pos : current_pos + remainder] = (
                    embedding[:remainder] * weight
                )
...
async def get_embedding_service() -> LocalEmbeddingService:
    ...
    if _embedding_service is None:
        ...
        _embedding_service = LocalEmbeddingService(
            model_name=model_name, use_mps=use_mps
        )
...
async def test_embedding_service():
    ...
    print(
        f"✅ Single embedding: {result.embedding.shape}, confidence: {result.confidence:.3f}"
    )
    ...
    for i, result in enumerate(batch_results):
        print(
            f"   {i+1}. Shape: {result.embedding.shape}, Cache hit: {result.cache_hit}"
        )
    ...
    print(
        f"   Dimensions: {health['actual_dimensions']} → {health['target_dimensions']}"
    )
