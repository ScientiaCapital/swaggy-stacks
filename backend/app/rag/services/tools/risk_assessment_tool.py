...
class RiskAssessmentTool(AgentTool):
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=False, data=None, error=f"Risk assessment failed: {str(e)}")
    
    async def _validate_order(self, risk_manager: RiskManager, parameters: Dict[str, Any]) -> ToolResult:
    ...
    async def _calculate_position_size(self, risk_manager: RiskManager, parameters: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            result_data = {
                ...
                "account_percentage": (actual_position_size / account_value) * 100 if account_value > 0 else 0,
                "suggested_stop_loss": suggested_stop_loss,
                "suggested_take_profit": suggested_take_profit,
                "max_risk_amount": actual_position_size * risk_manager.stop_loss_percentage,
                "inputs": {
                    "symbol": symbol,
                    "price": price,
                    "account_value": account_value,
    ...
    async def _check_risk_limits(self, risk_manager: RiskManager, parameters: Dict[str, Any]) -> ToolResult:
        """Check current risk limit violations"""
        try:
            current_positions = parameters.get("current_positions", [])
            account_value = float(parameters.get("account_value", 100000))  # Default value
            daily_pnl = float(parameters.get("daily_pnl", 0.0))
            
            violations = risk_manager.check_risk_limits(current_positions, account_value, daily_pnl)
    ...
    async def _get_risk_summary(self, risk_manager: RiskManager, parameters: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            risk_summary = risk_manager.get_risk_summary(current_positions, account_value, daily_pnl)
