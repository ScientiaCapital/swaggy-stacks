...
class WyckoffAnalyzer(BaseAnalyzer):
    ...
    def __init__(self):
        super().__init__()
        self.phases = {
            'accumulation': ['preliminary_support', 'selling_climax', 'automatic_reaction', 'secondary_test', 'spring'],
            'markup': ['sign_of_strength', 'last_point_of_support', 'backup_to_edge'],
            'distribution': ['preliminary_supply', 'buying_climax', 'automatic_reaction', 'secondary_test', 'upthrust'],
    ...
    async def analyze(self, data: List[Dict], lookback_period: int, min_strength: float, **kwargs) -> Dict[str, Any]:
        ...
        try:
            if not self._validate_data(data, min_length=30):
                return {"patterns": [], "error": "Insufficient data for Wyckoff analysis"}
            
            df = self._prepare_dataframe(data)
            
            # Check if volume data is available
            if 'volume' not in df.columns:
                return {"patterns": [], "error": "Volume data required for Wyckoff analysis"}
    ...
    def _find_accumulation_phase(self, df, min_strength: float) -> List[Dict]:
        ...
        if price_stability < 0.03 and volume_trend < -0.1:  # Low volatility, declining volume
            patterns.append({
                ...
                "characteristics": {
                    ...
                    "phase_confidence": "medium" if price_stability < 0.02 else "low"
                },
        ...
        if price_stability < 0.04 and volume_analysis['high_volume_days'] > 0.3:
            patterns.append({
                ...
                "characteristics": {
                    ...
                    "phase_confidence": "high" if volume_analysis['high_volume_days'] > 0.4 else "medium"
                },
    ...
    def _identify_springs(self, df, swings: Dict) -> List[Dict]:
        ...
        for i, low in enumerate(recent_lows[:-1]):
            ...
            if (next_low['price'] < low['price'] * 0.98 and  # Break below previous low
                current_price > next_low['price'] * 1.02):   # Recovery above the low
    ...
    def _identify_upthrusts(self, df, swings: Dict) -> List[Dict]:
        ...
        for i, high in enumerate(recent_highs[:-1]):
            ...
            if (next_high['price'] > high['price'] * 1.02 and  # Break above previous high
                current_price < next_high['price'] * 0.98):   # Failure below the high
    ...
    def _analyze_volume_pattern(self, df) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'volume_classification': self._classify_volume_pattern(recent_volume, avg_volume)
            }
        except:
    ...
    def _identify_key_levels(self, df) -> Dict[str, float]:
        ...
        try:
            ...
            if swings['highs']:
                ...
                levels['secondary_resistance'] = sorted(recent_highs, reverse=True)[1] if len(recent_highs) > 1 else None
            
            if swings['lows']:
                recent_lows = [l['price'] for l in swings['lows'][-3:]]
                levels['support'] = min(recent_lows)
                levels['secondary_support'] = sorted(recent_lows)[1] if len(recent_lows) > 1 else None
