...
class CachedAnalyzer:
    ...
    async def analyze(
        ...
        try:
            ...
            cache_key = self._generate_cache_key(
                data, lookback_period, min_strength, **kwargs
            )
            ...
            if cached_result is not None:
                logger.debug(
                    f"Cache HIT for {self.analyzer.__class__.__name__}: {cache_key}"
                )
            ...
            logger.debug(
                f"Cache MISS for {self.analyzer.__class__.__name__}: {cache_key}"
            )
            ...
            if result and not result.get("error"):
                ...
                logger.debug(
                    f"Cached result for {self.analyzer.__class__.__name__}: {cache_key}"
                )
            else:
            ...
            logger.error(
                f"Cached analysis error for {self.analyzer.__class__.__name__}: {e}"
            )
    ...
    async def clear_cache(self, pattern: Optional[str] = None) -> int:
        ...
        try:
            cache_pattern = (
                pattern
                or f"{self.cache_name}_{self.analyzer.__class__.__name__.lower()}*"
            )
            cleared_count = await self.cache.clear(cache_pattern)
            logger.info(
                f"Cleared {cleared_count} cache entries for {self.analyzer.__class__.__name__}"
            )
            return cleared_count
        except Exception as e:
            logger.error(
                f"Failed to clear cache for {self.analyzer.__class__.__name__}: {e}"
            )
            return 0
        ...
        class CachedAnalyzerClass(analyzer_class):
            ...
            async def analyze(
                ...
                **kwargs,
                ...
                return await self._cached_wrapper.analyze(
                    data, lookback_period, min_strength, **kwargs
                )
