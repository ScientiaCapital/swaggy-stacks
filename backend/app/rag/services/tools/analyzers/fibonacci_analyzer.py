...
class FibonacciAnalyzer(BaseAnalyzer):
    ...
    async def analyze(self, data: List[Dict], lookback_period: int, min_strength: float, **kwargs) -> Dict[str, Any]:
        ...
        try:
            if not self._validate_data(data):
                return {"patterns": [], "error": "Insufficient data for Fibonacci analysis"}
            ...
            if trend_direction == "bullish":
                patterns_found.extend(self._analyze_bullish_fibonacci(
                    recent_low, recent_high, current_price, min_strength
                ))
            elif trend_direction == "bearish":
                patterns_found.extend(self._analyze_bearish_fibonacci(
                    recent_high, recent_low, current_price, min_strength
                ))
            
            # Add extension levels
            patterns_found.extend(self._calculate_extensions(
            ...
            return {
                ...
                "in_golden_zone": self._is_in_golden_zone(current_price, recent_low, recent_high, trend_direction),
                "golden_zone_level": self._calculate_golden_zone_level(recent_low, recent_high, trend_direction)
            }
            
        except Exception as e:
            return {"patterns": [], "error": f"Fibonacci analysis failed: {str(e)}"}
    
    def _analyze_bullish_fibonacci(self, swing_low: Dict, swing_high: Dict, current_price: float, min_strength: float) -> List[Dict]:
            ...
            if distance_from_fib < 0.02:  # Within 2% of Fibonacci level
                ...
                if strength >= min_strength:
                    pattern = {
                        ...
                        "type": "support" if level != self.GOLDEN_ZONE else "golden_zone_support",
                        "level": fib_price,
                        "ratio": level,
                        "strength": strength,
                        "signal": "strong_support" if level == self.GOLDEN_ZONE else "support",
                        "is_golden_zone": level == self.GOLDEN_ZONE,
                        "entry_zone": fib_price,
                        "stop_loss": swing_low['price'] * 0.98,  # 2% below swing low
                        "target": swing_high['price'] * 1.05 if level == self.GOLDEN_ZONE else swing_high['price']
                    }
                    
                    if level == self.GOLDEN_ZONE:
                        pattern["confluence_note"] = "High probability reversal zone - ideal for entry"
                    
                    patterns.append(pattern)
        
        return patterns
    
    def _analyze_bearish_fibonacci(self, swing_high: Dict, swing_low: Dict, current_price: float, min_strength: float) -> List[Dict]:
        ...
        for level in self.RETRACEMENT_LEVELS:
            ...
            if distance_from_fib < 0.02:  # Within 2% of Fibonacci level
                ...
                if strength >= min_strength:
                    pattern = {
                        "name": f"Fibonacci Retracement {level:.1%}",
                        "type": "resistance" if level != self.GOLDEN_ZONE else "golden_zone_resistance",
                        "level": fib_price,
                        "ratio": level,
                        "strength": strength,
                        "signal": "strong_resistance" if level == self.GOLDEN_ZONE else "resistance",
                        "is_golden_zone": level == self.GOLDEN_ZONE,
                        "entry_zone": fib_price,
                        "stop_loss": swing_high['price'] * 1.02,  # 2% above swing high
                        "target": swing_low['price'] * 0.95 if level == self.GOLDEN_ZONE else swing_low['price']
                    }
    ...
    def _calculate_extensions(self, swing_low: Dict, swing_high: Dict, current_price: float, trend_direction: str) -> List[Dict]:
        ...
        for level in self.EXTENSION_LEVELS:
            if trend_direction == "bullish":
                ...
                if extension_price > current_price * 1.01:  # Only show extensions above current price
                    patterns.append({
                ...
                if extension_price < current_price * 0.99:  # Only show extensions below current price
                    patterns.append({
                        "name": f"Fibonacci Extension {level:.1%}",
                        "type": "target",
                        "level": extension_price,
    ...
    def _is_in_golden_zone(self, current_price: float, swing_low: Dict, swing_high: Dict, trend_direction: str) -> bool:
    ...
    def _calculate_golden_zone_level(self, swing_low: Dict, swing_high: Dict, trend_direction: str) -> float:
