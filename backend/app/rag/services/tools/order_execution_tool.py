...
class OrderExecutionTool(AgentTool):
    ...
    async def _execute_simple_order(
        self, client: AlpacaClient, parameters: Dict[str, Any]
    ) -> ToolResult:
    ...
    async def _execute_bracket_order(
        self, order_manager: OrderManager, parameters: Dict[str, Any]
    ) -> ToolResult:
    ...
    async def _create_trailing_stop(
        ...
        try:
            ...
            trailing_result = await order_manager.create_trailing_stop(
                symbol=symbol, quantity=quantity, trail_percent=trail_percent, atr=atr
            )
