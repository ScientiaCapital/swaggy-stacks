...
class LangChainToolRegistry(BaseTradingToolRegistry):
    ...
    def __init__(self):
        ...
        self.permission_hierarchy = {
            ...
            PermissionLevel.ANALYSIS: [PermissionLevel.READ_ONLY, PermissionLevel.ANALYSIS],
            PermissionLevel.RISK_ASSESSMENT: [
                PermissionLevel.READ_ONLY, PermissionLevel.ANALYSIS, PermissionLevel.RISK_ASSESSMENT
            ],
            PermissionLevel.EXECUTION: [
                PermissionLevel.READ_ONLY, PermissionLevel.ANALYSIS, 
                PermissionLevel.RISK_ASSESSMENT, PermissionLevel.EXECUTION
            ],
        ...
        async def wrapped_func(*args, **kwargs) -> str:
            ...
            try:
                ...
                if metadata.fallback_tool and metadata.fallback_tool in self.tools:
                    ...
                    fallback_tool = self.tools[metadata.fallback_tool]
    ...
    async def _assess_trade_risk_wrapper(self, trade_data: str) -> str:
        ...
        try:
            ...
            symbol, action, quantity_str = parts[0].strip(), parts[1].strip(), parts[2].strip()
    ...
    async def _update_tool_metrics(self, tool_name: str, success: bool, execution_time: float) -> None:
        ...
        if tool_name in self.metadata:
            ...
            metadata.success_rate = (
                ...
                (1 - alpha) * metadata.success_rate
            )
            
            # Update average execution time
            metadata.avg_execution_time = (
                alpha * execution_time + 
                (1 - alpha) * metadata.avg_execution_time
            )
