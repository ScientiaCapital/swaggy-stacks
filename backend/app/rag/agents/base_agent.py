...
class BaseTradingAgent(ABC):
    ...
    def __init__(
        ...
        self.db_connection_string = db_connection_string or "postgresql://localhost/swaggy_rag"
    ...
    async def find_similar_patterns(
        ...
        try:
            ...
            async with self._get_db_connection() as conn:
                ...
                """, self.agent_name, pattern_embedding.tolist(), similarity_threshold, limit)
    ...
    async def learn_from_outcome(self, outcome: LearningOutcome) -> None:
        ...
        try:
            ...
            await self._store_learned_pattern(pattern_features, embedding_result.embedding, outcome)
            
            # Update performance statistics
            await self._update_performance_stats(outcome)
            
            logger.info(f"✅ {self.agent_name} learned from {outcome.actual_outcome} trade")
        ...
        try:
            async with self._get_db_connection() as conn:
                ...
                if patterns:
                    ...
                    self.performance_stats['patterns_learned'] = pattern_info['pattern_count']
                    logger.info(f"Loaded {pattern_info['pattern_count']} existing patterns "
                              f"for {self.agent_name} (avg success rate: {pattern_info['avg_success_rate']:.2%})")
                
        except Exception as e:
            logger.warning(f"Could not load existing patterns for {self.agent_name}: {e}")
    ...
    async def _update_performance_stats(self, outcome: LearningOutcome) -> None:
        ...
        self.performance_stats['avg_confidence'] = (
            (current_avg * (total_signals - 1) + new_confidence) / total_signals
        )
    ...
    async def get_performance_summary(self) -> Dict[str, Any]:
        ...
        try:
            async with self._get_db_connection() as conn:
                ...
                """, self.agent_name, self.strategy_type)
            ...
            logger.error(f"Error getting performance summary for {self.agent_name}: {e}")
            return self.performance_stats.copy()
