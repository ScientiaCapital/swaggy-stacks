...
class MarkovStrategy(StrategyPlugin):
    ...
    def analyze_market(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        next_state_probs = transition_matrix[current_state] if len(transition_matrix) > current_state else np.zeros(self.n_states)
    ...
    def generate_signal(self, analysis: Dict[str, Any], market_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        if 'error' in analysis:
            return {'action': 'HOLD', 'confidence': 0.0, 'reasoning': 'Insufficient data for Markov analysis'}
...
class FibonacciStrategy(StrategyPlugin):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        super().__init__(config)
        self.retracement_ratios = config.get('retracement_ratios', [0.236, 0.382, 0.5, 0.618, 0.786])
    ...
    def generate_signal(self, analysis: Dict[str, Any], market_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        confidence = max(0.0, 1.0 - (distance * 100))  # Higher confidence when closer to levels
...
class ConsolidatedStrategyAgent(BaseTradingAgent):
    ...
    def __init__(self, 
        ...
        for strategy_name in strategies_to_load:
            if strategy_name in self.AVAILABLE_STRATEGIES:
                ...
                self.strategies[strategy_name] = self.AVAILABLE_STRATEGIES[strategy_name](config)
        ...
        try:
            ...
            for strategy_name, strategy in self.strategies.items():
                try:
                    ...
                    logger.warning(f"Strategy {strategy_name} analysis failed", error=str(e))
    ...
    def _build_consensus(self, strategy_signals: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        for strategy_name, signal in strategy_signals.items():
            ...
            reasonings.append(f"{strategy_name}: {signal.get('reasoning', 'No reason')}")
        ...
        if self.consensus_method == 'weighted_average':
            ...
            buy_weight = sum(conf for action, conf in zip(actions, confidences) if action == 'BUY')
            sell_weight = sum(conf for action, conf in zip(actions, confidences) if action == 'SELL')
            hold_weight = sum(conf for action, conf in zip(actions, confidences) if action == 'HOLD')
        ...
        return {
            ...
            'reasoning': f"Consensus ({self.consensus_method}): " + " | ".join(reasonings)
        }
    
    def _extract_combined_features(self, strategy_results: Dict, market_data: Dict) -> Dict[str, Any]:
...
def create_multi_strategy_agent(strategies: List[str] = None, **kwargs) -> ConsolidatedStrategyAgent:
...
__all__ = [
    ...
    'create_markov_agent', 'create_wyckoff_agent', 'create_fibonacci_agent', 'create_multi_strategy_agent',
    'MarkovTradingAgent', 'WyckoffAgent', 'FibonacciAgent'  # Backwards compatibility
]