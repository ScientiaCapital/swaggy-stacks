...
from app.rag.services.tool_registry import get_tool_registry, PermissionLevel
...
class TradingWorkflow:
    ...
    async def _memory_retrieval_node(self, state: TradingState) -> TradingState:
        ...
        await self.context_builder.add_ai_message(
            state, f"Memory retrieval completed - Found {memory_count} similar historical patterns"
        )
    ...
    async def _strategy_evaluation_node(self, state: TradingState) -> TradingState:
        ...
        market_data = {
            "symbol": state["symbol"],
            "prices": [state["market_data"].current_price] if state["market_data"].current_price else [],
        ...
        await self.context_builder.add_ai_message(
            state, f"Strategy evaluation completed - Generated {signal_count} trading signals"
        )
    ...
    async def _risk_assessment_node(self, state: TradingState) -> TradingState:
        ...
        await self.context_builder.add_ai_message(
            state, f"Risk assessment completed - Status: {risk_status} (Risk Score: {risk_score:.1%})"
        )
    ...
    async def _decision_synthesis_node(self, state: TradingState) -> TradingState:
        ...
        await self.context_builder.add_ai_message(
            state, f"Decision synthesis completed - Final decision: {final_decision} "
    ...
    async def _execution_planning_node(self, state: TradingState) -> TradingState:
        ...
        if state["final_decision"] and state["final_decision"] != "HOLD":
            ...
            if state["execution_plan"]:
                ...
                await self.context_builder.add_ai_message(
                    state, f"Execution plan created - {plan.action} {plan.quantity:.2f} shares "
                           f"via {plan.order_type} order"
                )
            else:
                await self.context_builder.add_ai_message(state, "No execution plan needed (HOLD decision)")
        else:
            await self.context_builder.add_ai_message(state, "No execution required - Decision: HOLD")
    ...
    def _risk_gate_condition(self, state: TradingState) -> str:
        ...
        if not state["risk_approval"] and any(
            ...
            state["warnings"].append("Risk gate: High-confidence signals require risk approval")
            return "halt"
