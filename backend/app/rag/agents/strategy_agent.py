...
class WyckoffStrategy(StrategyPlugin):
    ...
    def analyze_market(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                len(prices) < self.volume_lookback
                or len(volumes) < self.volume_lookback
            ):
...
class StrategyAgent(BaseTradingAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Validate strategies
        invalid_strategies = [
            s for s in self.strategies if s not in self.AVAILABLE_STRATEGIES
        ]
        ...
        self.logger.info(
            f"Refactored StrategyAgent initialized with {len(self.strategies)} strategies"
        )
    ...
    def _initialize_modular_strategy_engines(self) -> None:
        ...
        try:
            ...
            for strategy in self.strategies:
                ...
                if strategy in strategy_class_map:
                    self.strategy_engines[strategy] = strategy_class_map[strategy](
                        config
                    )
        ...
        self.logger.info(
            f"Initialized {len(self.strategy_engines)} modular strategy engines"
        )
    ...
    async def submit_trading_idea_for_learning(
        ...
        try:
            ...
            result = await self.backtest_service.submit_trading_idea(
                agent_id=self.agent_id, trading_idea=trading_idea
            )
            ...
            self.logger.info(
                ...
                symbol=symbol,
                strategy=strategy,
                performance_rank=result.get("feedback", {}).get(
            ...
            self.logger.error(
                "Error submitting trading idea for learning", error=str(e)
            )
            return {"error": str(e)}
    ...
    async def optimize_strategy_parameters(
        ...
        try:
            ...
            optimization_result = (
                await self.backtest_service.optimize_strategy_parameters(
                    agent_id=self.agent_id,
            ...
            self.logger.info(
                ...
                combinations_tested=optimization_result.get(
    ...
    async def get_learning_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = await self.backtest_service.get_agent_learning_summary(
                agent_id=self.agent_id, days_lookback=30
            )
    ...
    def _extract_entry_conditions(
        self, market_data: Dict[str, Any], strategy: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if strategy == "technical":
                conditions.update(
                    {
                        ...
                        "macd_signal": (
                            "bullish"
                            if market_data.get("macd_histogram", 0) > 0
                            else "bearish"
                        ),
                ...
                conditions.update(
                    {
                        "pattern_confidence": market_data.get(
                            "pattern_confidence", 0.6
                        ),
                        "trend_alignment": market_data.get("trend_context", "neutral"),
                        "volume_confirmation": market_data.get(
                ...
                conditions.update(
                    {
                        "consensus_strength": market_data.get(
                            "consensus_strength", 0.5
                        ),
                        "strategy_agreement": market_data.get(
    ...
    def _define_parameter_space(
        self, strategy_name: str, current_parameters: Dict[str, Any]
        ...
        try:
            if strategy_name == "technical":
                ...
                return {
                    ...
                    "strategy_weight_technical": {
                        "type": "float",
                        "min": 0.1,
                        "max": 0.5,
                        "steps": 4,
                    },
                    "strategy_weight_pattern": {
                        "type": "float",
                        "min": 0.1,
                        "max": 0.5,
                        "steps": 4,
        ...
        for strategy_name, strategy_engine in self.strategy_engines.items():
            if hasattr(strategy_engine, "get_tools"):
                try:
                    ...
                    self.logger.warning(
                        f"Failed to get tools from {strategy_name}: {e}"
                    )
        ...
        try:
            ...
            if len(parts) > 1 and parts[1]:
                ...
                market_conditions = {
                    ...
                    "trend": (
                        parts[1]
                        if parts[1] in ["bullish", "bearish", "neutral"]
                        else "neutral"
                    ),
                }

            insights = await self.get_pattern_learning_insights(
                pattern_name=pattern_name, market_conditions=market_conditions
            )
    ...
    async def _get_learning_summary_tool(self, input_str: str = "") -> str:
        ...
        try:
            ...
            summary.get("strategy_performance", {})
    ...
    async def analyze_market(self, market_data: Dict[str, Any]) -> TradingSignal:
        ...
        try:
            ...
            if "composite" in self.strategy_engines:
                ...
                if hasattr(composite, "analyze_market") and hasattr(
                    composite, "generate_signal"
                ):
                    analysis = composite.analyze_market(market_data)
                    if "error" not in analysis:
                        signal_data = composite.generate_signal(analysis, market_data)
                        return self._convert_to_trading_signal(
                            signal_data, symbol, {"composite_analysis": analysis}
                        )
    ...
    async def _fallback_modular_analysis(
        ...
        for strategy_name, strategy_engine in self.strategy_engines.items():
            if hasattr(strategy_engine, "analyze_market") and hasattr(
                strategy_engine, "generate_signal"
            ):
                try:
                    analysis = strategy_engine.analyze_market(market_data)
                    if "error" not in analysis:
                        signal_data = strategy_engine.generate_signal(
                            analysis, market_data
                        )
                        return self._convert_to_trading_signal(
                            signal_data, symbol, {f"{strategy_name}_analysis": analysis}
                        )
                except Exception as e:
    ...
    def _convert_to_trading_signal(
        self, signal_data: Dict[str, Any], symbol: str, metadata: Dict[str, Any]
    ) -> TradingSignal:
        """Convert strategy signal to TradingSignal object"""
        from app.rag.models.trading_models import SignalType, TradingSignal
    ...
    async def _execute_langgraph_workflow(self, symbol: str) -> str:
        ...
        try:
            if not hasattr(self, "workflow_engine"):
                from app.rag.agents.langgraph.trading_workflow import (
            ...
            initial_state = {
                ...
                "workflow_metadata": {
                    "initiated_by": f"RefactoredStrategyAgent_{self.agent_name}",
            ...
            analysis_summary = {
                ...
                "recommended_action": final_state.get("final_decision", {}).get(
                    "action", "HOLD"
                ),
            ...
            return json.dumps(
                {"error": f"LangGraph workflow failed: {str(e)}", "symbol": symbol}
            )
...
ConsolidatedStrategyAgent = (
    StrategyAgent  # For existing code that imports ConsolidatedStrategyAgent
)
