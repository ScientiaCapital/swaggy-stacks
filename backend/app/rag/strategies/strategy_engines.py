...
class WyckoffStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.price_action_sensitivity = self.config.get('price_action_sensitivity', 0.02)
    ...
    async def analyze(self, symbol: str, market_data: Dict[str, Any]) -> Optional[StrategySignal]:
        ...
        try:
            ...
            if current_phase == 'accumulation' and volume_ratio > self.volume_threshold:
                ...
                rationale = "Accumulation phase with high volume suggests institutional buying"
            elif current_phase == 'distribution' and volume_ratio > self.volume_threshold:
                direction = 'SELL'
                confidence = min(0.9, price_strength + 0.1)
                entry_price = current_price
                stop_loss = current_price * 1.04
                take_profit = current_price * 0.88
                rationale = "Distribution phase with high volume suggests institutional selling"
            else:
    ...
    def get_parameter_space(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            'price_action_sensitivity': {'type': 'float', 'min': 0.01, 'max': 0.05, 'steps': 5}
        }
    ...
    async def analyze(self, symbol: str, market_data: Dict[str, Any]) -> Optional[StrategySignal]:
        ...
        try:
            ...
            for level_name, level_price in fib_levels.items():
                if abs(current_price - level_price) / current_price <= self.fib_tolerance:
            ...
            if trend == 'uptrend' and level_name in ['61.8%', '78.6%']:
                ...
                rationale = f"Bounce expected at {level_name} Fibonacci level in uptrend"
            elif trend == 'downtrend' and level_name in ['23.6%', '38.2%']:
                direction = 'SELL'
                entry_price = current_price
                stop_loss = current_price * 1.03
                take_profit = current_price * 0.94
                rationale = f"Rejection expected at {level_name} Fibonacci level in downtrend"
            else:
                return None
...
class StrategyEngineManager:
    ...
    def _initialize_strategies(self):
        ...
        try:
            ...
            self.strategies['fibonacci'] = FibonacciStrategy(self.config.get('fibonacci', {}))
    ...
    def _confidence_weighted_consensus(self, signals: List[StrategySignal]) -> Optional[StrategySignal]:
        ...
        try:
            ...
            return StrategySignal(
                ...
                indicators_used=list(set().union(*[s.indicators_used for s in signals])),
                market_context={
    ...
    def _majority_vote_consensus(self, signals: List[StrategySignal]) -> Optional[StrategySignal]:
    ...
    def _simple_average_consensus(self, signals: List[StrategySignal]) -> Optional[StrategySignal]:
