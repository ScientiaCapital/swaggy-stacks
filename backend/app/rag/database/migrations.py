...
class RAGDatabaseMigrator:
    ...
    async def _run_migration(self, conn: asyncpg.Connection, migration_name: str) -> None:
    ...
    async def _create_base_tables(self, conn: asyncpg.Connection) -> None:
        ...
        for line in schema_sql.split('\n'):
            ...
            if (line.startswith('CREATE EXTENSION') or 
                line.startswith('CREATE INDEX') or
                line.startswith('CREATE VIEW') or
                line.startswith('CREATE OR REPLACE FUNCTION') or
                line.startswith('SELECT create_hypertable')):
                continue
    ...
    async def _create_hypertables(self, conn: asyncpg.Connection) -> None:
        ...
        for hypertable in hypertables:
            try:
                ...
                logger.warning(f"  ⚠️  Hypertable warning for {hypertable['table']}: {e}")
    ...
    async def validate_setup(self) -> Dict[str, Any]:
        ...
        async with self.get_connection() as conn:
            try:
                ...
                validation_results['extensions'] = [ext['extname'] for ext in extensions]
                ...
                """)
                validation_results['vector_indexes'] = [idx['indexname'] for idx in vector_indexes]
                ...
                try:
                    ...
                    """)
                    validation_results['hypertables'] = [ht['hypertable_name'] for ht in hypertables]
                except:
...
async def main():
    ...
    if not os.getenv('DATABASE_URL'):
        ...
        print("💡 Example: export DATABASE_URL='postgresql://user:pass@localhost/swaggy_rag'")
        sys.exit(1)
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--validate":
        ...
        if success_count == total_count:
            ...
            print(f"  Extensions: {len(validation_results.get('extensions', []))} enabled")
            print(f"  Tables: {len(validation_results.get('tables', []))} created")
            print(f"  Vector indexes: {len(validation_results.get('vector_indexes', []))} active")
            print(f"  Hypertables: {len(validation_results.get('hypertables', []))} configured")
            print(f"  Vector search: {'✅' if validation_results.get('vector_search') else '❌'}")
            print(f"  Similarity function: {'✅' if validation_results.get('similarity_function') else '❌'}")
        else:
