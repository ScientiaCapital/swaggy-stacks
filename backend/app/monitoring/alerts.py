...
from .health_checks import ComponentType, HealthStatus, SystemHealthStatus
...
class AlertManager:
    ...
    def __init__(
        self, risk_thresholds: Optional[RiskThresholdConfig] = None, settings=None
    ):
    ...
    async def _check_system_alerts(self, health_status: SystemHealthStatus):
        ...
        system_rules = [
            rule for rule in self.alert_rules if "overall_status" in rule.condition
        ]
        ...
        for rule in system_rules:
            ...
            if (
                rule.condition == "overall_status == 'critical'"
                and health_status.overall_status == HealthStatus.CRITICAL
            ):
                should_alert = True
            elif (
                rule.condition == "overall_status == 'degraded'"
                and health_status.overall_status == HealthStatus.DEGRADED
            ):
                should_alert = True
                ...
                if self._should_send_alert(rule.name, rule.cooldown_minutes):
                    alert = Alert(
                        ...
                        details={
                            ...
                            "healthy_components": health_status.summary.get(
                                HealthStatus.HEALTHY, 0
                            ),
                            "degraded_components": health_status.summary.get(
                                HealthStatus.DEGRADED, 0
                            ),
                            "critical_components": health_status.summary.get(
    ...
    async def _check_component_alerts(self, component):
        ...
        for rule in component_rules:
            ...
            if "component_status" in rule.condition:
                if (
                    rule.condition == "component_status == 'critical'"
                    and component.status == HealthStatus.CRITICAL
                ):
                    should_alert = True
                elif (
                    rule.condition == "component_status == 'degraded'"
                    and component.status == HealthStatus.DEGRADED
                ):
                    should_alert = True
    ...
    async def _resolve_inactive_alerts(self, health_status: SystemHealthStatus):
        ...
        for alert_key, alert in self.active_alerts.items():
            ...
            # Check if component alerts should be resolved
                ...
                matching_components = [
                    c for c in health_status.components if c.component == component_name
                ]
    ...
    def configure_prometheus_alerts(self) -> List[AlertRule]:
        ...
        logger.info(
            f"Configured {len(prometheus_rules)} Prometheus-based alert rules with configurable thresholds"
        )
