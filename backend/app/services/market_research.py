...
from app.core.exceptions import MCPError, MCPConnectionError
from app.mcp.orchestrator import get_mcp_orchestrator, MCPOrchestrator, MCPServerType
...
class MarketResearchService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            tavily_available = await self._orchestrator.is_server_available(MCPServerType.TAVILY)
            sequential_available = await self._orchestrator.is_server_available(MCPServerType.SEQUENTIAL_THINKING)
            
            logger.info("MarketResearchService initialization complete",
    ...
    async def complex_analysis_workflow(
        ...
        if use_cache and cache_key in self._analysis_cache:
            ...
            if datetime.now() - cached_result.timestamp < self._cache_duration:
                logger.info("Using cached complex analysis", symbol=symbol, analysis_type=analysis_type)
                return cached_result
        ...
        try:
            ...
            analysis_data = await self._process_sequential_thinking_result(thinking_result, symbol, analysis_type)
            ...
            logger.info("Complex analysis workflow completed",
                       symbol=symbol,
                       analysis_type=analysis_type,
            ...
            logger.error("Complex analysis workflow failed",
                        symbol=symbol,
                        analysis_type=analysis_type,
                        error=str(e))
    ...
    async def integrated_strategy_analysis(
        ...
        logger.info("Starting integrated strategy analysis",
                   symbol=symbol,
        ...
        if include_complex_analysis:
            analysis_tasks.append(self.complex_analysis_workflow(
                symbol, 
                "integrated_strategy_analysis", 
                analysis_complexity
            ))
        ...
        try:
            ...
            result_index = 0
            if include_sentiment:
                sentiment_result = results[result_index] if not isinstance(results[result_index], Exception) else None
                result_index += 1
            
            if include_complex_analysis:
                complex_result = results[result_index] if not isinstance(results[result_index], Exception) else None
                result_index += 1
            
            if include_technical and markov_task:
                markov_result = results[result_index] if not isinstance(results[result_index], Exception) else None
            ...
            logger.info("Integrated strategy analysis completed",
                       symbol=symbol,
            ...
            logger.error("Integrated strategy analysis failed",
                        symbol=symbol,
                        error=str(e))
    ...
    async def _process_tavily_sentiment(self, tavily_result: Dict[str, Any], symbol: str) -> Dict[str, Any]:
    ...
    async def _process_sequential_thinking_result(
        ...
        try:
            ...
            return {
                ...
                "supporting_evidence": [
                    {"type": "technical", "indicator": "RSI", "value": 65.2, "interpretation": "Not overbought"},
                    {"type": "fundamental", "metric": "P/E", "value": 18.5, "interpretation": "Reasonable valuation"},
                    {"type": "sentiment", "metric": "news_sentiment", "value": 0.15, "interpretation": "Slightly positive"}
                ],
                "risk_factors": [
    ...
    async def _generate_trading_recommendation(
        ...
        if sentiment:
            ...
            if sentiment.sentiment in [SentimentLevel.BULLISH, SentimentLevel.EXTREMELY_BULLISH]:
                bullish_signals += weight
            elif sentiment.sentiment in [SentimentLevel.BEARISH, SentimentLevel.EXTREMELY_BEARISH]:
        ...
        if complex_analysis and complex_analysis.confidence > 0:
            ...
            if any(word in conclusion_lower for word in ["bullish", "buy", "positive", "upward"]):
                bullish_signals += weight
            elif any(word in conclusion_lower for word in ["bearish", "sell", "negative", "downward"]):
                bearish_signals += weight
        ...
        return {
            ...
            "risk_management": {
                "stop_loss": 0.95 if action == "buy" else 1.05 if action == "sell" else None,
                "take_profit": 1.08 if action == "buy" else 0.92 if action == "sell" else None,
                "max_position_size": min(0.1, confidence * 0.15)  # Max 10%, scaled by confidence
            }
        }
    ...
    async def clear_cache(self):
        ...
        logger.info("Market research cache cleared")
