...
class MLTrainingPipeline:
    ...
    async def initialize_llm_predictor(self):
        ...
        try:
            ...
            logger.info("ML training pipeline initialized",
                       available_models=self.llm_predictor.available_models if self.llm_predictor else 0)
    ...
    async def create_model_version(
        ...
        try:
            ...
            existing_versions = self.db.query(MLModelVersion).filter(
    ...
    async def record_prediction(
        ...
        try:
            ...
            logger.info("Recorded ML prediction",
                       symbol=symbol, direction=predicted_direction, confidence=confidence_score)
            return prediction
    ...
    async def update_prediction_outcome(
        ...
        try:
            ...
            prediction_error = abs(prediction.predicted_return - actual_return) if prediction.predicted_return else None
            ...
            if direction_correct and prediction.confidence_score > 0.7:
                prediction.learning_weight = 1.2  # Higher weight for confident correct predictions
                ...
                prediction.learning_weight = 1.5   # Higher weight to learn from confident mistakes
            else:
            ...
            logger.info("Updated prediction outcome",
                       prediction_id=prediction_id, was_correct=direction_correct)
            return True
    ...
    async def get_model_performance_report(
        ...
        try:
            ...
            # Query predictions
            query = self.db.query(MLPrediction).join(MLModelVersion).filter(
            ...
            for model_key, data in model_performance.items():
                ...
                completed_predictions = [p for p in predictions_list if p.was_correct is not None]

                if completed_predictions:
                    accuracy = sum(1 for p in completed_predictions if p.was_correct) / len(completed_predictions)
                    avg_confidence = np.mean([float(p.confidence_score) for p in completed_predictions])
                    returns = [float(p.actual_return) for p in completed_predictions if p.actual_return]
    ...
    async def _check_retraining_trigger(self, model_version_id: int):
        ...
        try:
            ...
            recent_predictions = self.db.query(MLPrediction).filter(
                MLPrediction.model_version_id == model_version_id,
                MLPrediction.was_correct.isnot(None)
            ).order_by(MLPrediction.prediction_time.desc()).limit(
                self.training_config["performance_window"]
            ).all()
            ...
            baseline_accuracy = model_version.production_metrics.get('accuracy', 0.6) if model_version.production_metrics else 0.6
    ...
    async def _schedule_model_retraining(self, model_version_id: int):
        ...
        try:
            ...
            feedback_predictions = self.db.query(MLPrediction).filter(
                MLPrediction.model_version_id == model_version_id,
                MLPrediction.feedback_processed == False,
                MLPrediction.was_correct.isnot(None),
                MLPrediction.learning_weight > 0.5  # Only use high-quality feedback
            ...
            logger.info("Prepared training data for model retraining",
                       model_version_id=model_version_id,
    ...
    async def _update_model_metrics(self, model_version_id: int, predictions: List[MLPrediction]):
        ...
        try:
            ...
            accuracy = len(correct_predictions) / len(predictions) if predictions else 0.0
            ...
            logger.info("Updated model production metrics",
                       model_version_id=model_version_id,
                       accuracy=accuracy)
    ...
    async def cleanup_old_model_versions(self):
        ...
        try:
            retention_date = datetime.now() - timedelta(days=self.training_config["model_retention_days"])

            # Find old model versions (keep production and active models)
            old_versions = self.db.query(MLModelVersion).filter(
            ...
            for version in old_versions:
                ...
                prediction_count = self.db.query(MLPrediction).filter(
                    MLPrediction.model_version_id == version.id
                ).count()

                if prediction_count == 0:  # Safe to delete
                    self.db.delete(version)
                    logger.info("Deleted old model version",
                               model_name=version.model_name, version=version.version)
