...
class IndicatorPerformanceService:
        ...
        try:
            ...
            if self.metrics:
                self.metrics.indicator_signals_total.labels(
                    indicator=indicator_name, signal_type=signal_type
                ).inc()
    ...
    async def update_indicator_signal_outcome(
        ...
        try:
            performance = (
                self.db.query(IndicatorPerformance)
                .filter(IndicatorPerformance.id == performance_id)
                .first()
            )
    ...
    async def get_indicator_performance_report(
        ...
        try:
            ...
            if market_condition:
                query = query.filter(
                    IndicatorPerformance.market_condition == market_condition
                )
            ...
            for name, stats in indicator_stats.items():
                ...
                win_rate = (
                    (stats["wins"] / total_closed * 100) if total_closed > 0 else 0
                )
                ...
                report_data.append(
                    {
                        ...
                        "avg_pnl_per_trade": (
                            round(stats["total_pnl"] / total_closed, 2)
                            if total_closed > 0
                            else 0
                        ),
                    }
    ...
    async def get_indicator_market_condition_analysis(
        ...
        try:
            ...
            return {
                ...
                "total_signals_analyzed": sum(
                    perf["total_signals"] for perf in analysis
                ),
                "generated_at": datetime.utcnow().isoformat(),
            }
