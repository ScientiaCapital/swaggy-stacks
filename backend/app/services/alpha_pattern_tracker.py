...
class AlphaPatternTracker:
    ...
    async def record_pattern_detection(
        self, detection: PatternDetection, market_conditions: Dict[str, Any] = None
    ) -> str:
        """
        ...
        try:
            ...
            existing_pattern = (
                self.db.query(PatternPerformance)
                .filter(
                    and_(
                        PatternPerformance.pattern_signature == pattern_signature,
                        PatternPerformance.detection_timestamp
            ...
            if existing_pattern:
                self.logger.info(
                    f"Pattern already detected recently: {pattern_signature}"
                )
            ...
            pattern_record = PatternPerformance(
                ...
                market_volatility=(
                    market_conditions.get("volatility") if market_conditions else None
                ),
                market_regime=(
                    market_conditions.get("regime") if market_conditions else None
                ),
                volume_profile=(
                    market_conditions.get("volume_profile")
            ...
            asyncio.create_task(
                self._schedule_alpha_verification(str(pattern_record.id))
            )
    ...
    async def update_pattern_outcome(
        ...
        try:
            pattern = (
                self.db.query(PatternPerformance)
                .filter(PatternPerformance.id == pattern_id)
                .first()
            )
            ...
            pattern.pattern_score = self._calculate_pattern_score(
                pattern, alpha_metrics
            )
    ...
    async def generate_alpha_signal(
        ...
        try:
            signal_id = (
                f"{symbol}_{signal_type}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    async def get_llm_alpha_performance(
        self, llm_model: str, days_back: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            successful_patterns = len(
                [p for p in patterns if (p.alpha_generated or 0) > 0]
            )
            ...
            for pattern in patterns:
                ...
                pattern_breakdown[pattern_type]["total_alpha"] += (
                    pattern.alpha_generated or 0
                )
            ...
            for pattern_type in pattern_breakdown:
                ...
                pb["avg_alpha"] = (
                    pb["total_alpha"] / pb["count"] if pb["count"] > 0 else 0
                )
                pb["success_rate"] = (
                    pb["success_rate"] / pb["count"] if pb["count"] > 0 else 0
                )
    ...
    async def get_top_alpha_patterns(
        self, limit: int = 10, min_occurrences: int = 5
    ) -> List[Dict[str, Any]]:
        """
            ...
            top_patterns = (
                ...
                .filter(
                    and_(
                        ...
                        PatternLearning.avg_alpha_generated
                        > self.alpha_config["alpha_significance_threshold"],
                    )
                )
                .order_by(desc(PatternLearning.avg_alpha_generated))
                .limit(limit)
                .all()
            )
                ...
                patterns_list.append(
                    {
                        ...
                        "learning_confidence": round(
                            pattern.confidence_in_learning or 0, 3
                        ),
                    }
                )
    ...
    def _calculate_magnitude_accuracy(
        self, predicted: Optional[float], actual: float
    ) -> float:
    ...
    def _calculate_pattern_score(
        self, pattern: PatternPerformance, alpha_metrics: AlphaMetrics
    ) -> float:
        ...
        confidence_score = (
            ...
            else (1 - pattern.detection_confidence)
        )
    ...
    async def _update_llm_performance_metrics(
        ...
        try:
            ...
            metrics = (
                self.db.query(LLMPerformanceMetrics)
                .filter(
                    and_(
            ...
            metrics.avg_alpha_per_prediction = (
                metrics.total_alpha_generated / metrics.total_predictions
            )
    ...
    async def _update_pattern_learning(
        ...
        try:
            ...
            learning = (
                self.db.query(PatternLearning)
                .filter(PatternLearning.pattern_family == pattern.pattern_type)
                .first()
            )
            ...
            learning.success_rate = (
                learning.successful_occurrences / learning.total_occurrences
            )
            ...
            learning.confidence_in_learning = min(
                1.0, learning.total_occurrences / 50.0
    ...
    def _calculate_recent_trends(
        self, patterns: List[PatternPerformance]
    ) -> Dict[str, Any]:
        ...
        second_half_accuracy = np.mean(
            [p.prediction_accuracy or 0 for p in second_half]
        )

        return {
            "alpha_trend": (
                "improving" if second_half_alpha > first_half_alpha else "declining"
            ),
