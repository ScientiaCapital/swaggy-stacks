...
class MarkovCore:
    ...
    def __init__(self, lookback_period: int = 100, n_states: int = 5):
        ...
        logger.info("Markov core initialized", lookback_period=lookback_period, n_states=n_states)
    ...
    def predict_next_state(self, current_state: int, steps: int = 1) -> Tuple[int, float]:
...
class EnhancedMarkovSystem(MarkovCore):
    ...
    def __init__(self, lookback_period: int = 100, n_states: int = 5, **kwargs):
        ...
        # Enhanced parameters
        self.confidence_adjustment_factor = kwargs.get('confidence_adjustment_factor', 1.2)
    ...
    def analyze(self, price_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # Clean data
            if isinstance(price_data, pd.DataFrame):
                prices = price_data['close'].values if 'close' in price_data.columns else price_data.iloc[:, 0].values
            else:
                prices = np.array(price_data)
            
            cleaned_prices, cleaned_volumes = self.data_handler.validate_and_clean_data(prices)
    ...
    def _generate_enhanced_signals(self, price_data: pd.DataFrame, returns: pd.Series, states: pd.Series) -> Dict:
        ...
        if self.transition_matrix is not None:
            ...
            position_size = self.position_sizer.calculate_size(uncertainty, current_price)
            stop_loss = self.position_sizer.calculate_stop_loss(
                {'consensus': {'action': signal}}, 
                current_price
            )
        ...
        for period in self.lookback_periods:
            if len(prices) >= period:
                ...
                try:
                    ...
                    if len(states) > 0:
                        current_state = int(states.iloc[-1]) if not pd.isna(states.iloc[-1]) else 0
                        state_prob = self.state_probabilities[current_state] if self.state_probabilities is not None else 0
                    ...
                    logger.warning(f"Multi-timeframe analysis failed for period {period}", error=str(e))
                    continue
...
EnhancedMarkovTradingSystem = EnhancedMarkovSystem  # For enhanced_markov_system.py imports
