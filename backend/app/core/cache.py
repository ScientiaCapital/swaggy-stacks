...
class CacheMetrics:
    ...
    def _update_avg_time(self, cache_type: str, response_time: float):
        if cache_type == "l1":
            self.avg_l1_time = (
                self.avg_l1_time * (self.l1_hits - 1) + response_time
            ) / self.l1_hits
        else:
            self.avg_l2_time = (
                self.avg_l2_time * (self.l2_hits - 1) + response_time
            ) / self.l2_hits
...
class TTLCache:
    ...
    def __init__(
        ...
        if REDIS_AVAILABLE:
            try:
                ...
                logger.warning(
                    "Failed to initialize Redis client, falling back to L1 only",
                    error=str(e),
                )
        ...
        try:
            ...
            if cached_data is not None:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to deserialize cached value", key=key, error=str(e)
                    )
    ...
    async def _maybe_warm_key(self, key: str):
        ...
        async with self._warming_lock:
            if (
                key not in self._warming_keys
                and self.metrics.l1_hits > self.warming_threshold
            ):
        ...
        try:
            ...
            return {
                ...
                "l2_cache": {
                    ...
                    "working": l2_working,
                    "connected": (
                        redis_info.get("connected_clients", 0) > 0
                        if redis_info
                        else False
                    ),
                    "memory_usage": (
                        redis_info.get("used_memory_human", "unavailable")
                        if redis_info
                        else "unavailable"
                    ),
                    "ttl": self.l2_ttl,
