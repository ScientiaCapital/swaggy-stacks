...
class OrderManager:
        ...
        try:
            ...
            if not stop_loss_price:
                if atr:
                    ...
                    stop_loss_price = self._calculate_atr_stop_loss(entry_price, side, atr, multiplier=2.0)
                else:
            ...
            if not take_profit_price:
                ...
                take_profit_price = self.risk_manager.calculate_take_profit(entry_price, side)
    ...
    async def monitor_positions(self):
        ...
        try:
            ...
            for position in positions:
                ...
                current_price = quotes.get(symbol, {}).get('bid') or quotes.get(symbol, {}).get('ask')
                ...
                if stop_loss_triggered:
                    await self._execute_stop_loss(symbol, current_qty, current_price, "STOP_LOSS")
                elif take_profit_triggered:
                    await self._execute_take_profit(symbol, current_qty, current_price, "TAKE_PROFIT")
    ...
    def _calculate_atr_stop_loss(self, entry_price: float, side: str, atr: float, multiplier: float = 2.0) -> float:
    ...
    async def _execute_stop_loss(self, symbol: str, quantity: float, current_price: float, reason: str):
    ...
    async def _execute_take_profit(self, symbol: str, quantity: float, current_price: float, reason: str):
    ...
    async def cleanup_expired_orders(self, max_age_hours: int = 24):
        ...
        expired_positions = [
            symbol for symbol, pos_info in self.monitored_positions.items()
