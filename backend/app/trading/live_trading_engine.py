...
class LiveTradingEngine:
    ...
    def __init__(self, user_id: int, symbols: List[str], enable_ai: bool = True):
        ...
        self.market_hours = {"start": "09:30", "end": "16:00", "timezone": "US/Eastern"}
    ...
    async def start_trading(self):
        ...
        try:
            ...
            logger.info(
                "Connected to Alpaca account", account_value=account["portfolio_value"]
            )
    ...
    async def _analyze_and_trade_symbol(
        ...
        try:
            ...
            volatility = price_data["Close"].pct_change().rolling(20).std().iloc[
                -1
            ] * np.sqrt(252)
            ...
            market_data = {
                ...
                "high_52w": (
                    price_data["High"].rolling(252).max().iloc[-1]
                    if len(price_data) >= 252
                    else price_data["High"].max()
                ),
                "low_52w": (
                    price_data["Low"].rolling(252).min().iloc[-1]
                    if len(price_data) >= 252
            ...
            if self.enable_ai and self.ai_coordinator:
                try:
                    ...
                    ai_confidence = ai_analysis.get("strategy_signal", {}).get(
                        "confidence", 0.0
                    )
                    ...
                    if ai_confidence < 0.6:
                        logger.info(
                            "Low AI confidence, using fallback",
                            symbol=symbol,
                    ...
                    logger.error(
                        "AI analysis failed, using Markov fallback",
                        symbol=symbol,
                        error=str(e),
                    )
                    final_signal = markov_analysis["signal"]
    ...
    async def _execute_sell_signal(
        self, symbol: str, current_price: float, position: Dict, confidence: float
    ):
        ...
        try:
            ...
            sell_order = await self.alpaca_client.execute_order(
                symbol=symbol, qty=shares, side="sell", order_type="market"
            )
            ...
            # Calculate P&L
            entry_price = (
                float(position.get("cost_basis", 0)) / shares if shares > 0 else 0
            )
            ...
            return {
                ...
                "bollinger_bands": {
                    "upper": (
                        ...
                        else price_series.iloc[-1] * 1.02
                    ),
                    "lower": (
                        bb_lower
                        if not pd.isna(bb_lower)
                        else price_series.iloc[-1] * 0.98
                    ),
                },
            }
    ...
    def _calculate_markov_signals(self, price_data: pd.DataFrame, symbol: str) -> Dict:
        ...
        try:
            ...
            logger.error(
                "Error calculating Markov signals", symbol=symbol, error=str(e)
            )
            return {"signal": "HOLD", "confidence": 0.0, "strength": 0}
            ...
            for symbol in self.symbols:
                try:
                    ...
                    if not hist.empty:
                        ...
                        logger.debug(
                            "Market data retrieved", symbol=symbol, days=len(hist)
                        )
                    else:
                    ...
                    logger.error(
                        "Error getting market data", symbol=symbol, error=str(e)
                    )
        ...
        try:
            ...
            for order in orders:
                try:
                    ...
                    logger.info(
                        "Cancelled order", order_id=order["id"], symbol=order["symbol"]
                    )
                except Exception as e:
                    logger.error(
                        "Failed to cancel order", order_id=order["id"], error=str(e)
                    )
            ...
            for position in positions:
                try:
                    ...
                    logger.info(
                        "Emergency position closed", symbol=symbol, qty=qty, side=side
                    )
