...
class TradingManager:
    ...
    async def initialize(self, user_config: Optional[Dict] = None):
        ...
        async with self._lock:
            try:
                ...
                api_key = user_config.get('api_key') if user_config else settings.ALPACA_API_KEY
                secret_key = user_config.get('secret_key') if user_config else settings.ALPACA_SECRET_KEY
                ...
                self._risk_manager = RiskManager(
                    user_id=user_id,
                    account_balance=float(account_info.get('equity', self.account_size)),
                ...
                logger.info("TradingManager fully initialized", 
    ...
    def alpaca_client(self) -> AlpacaClient:
        ...
        if not self._alpaca_client:
            raise TradingError("TradingManager not initialized. Call initialize() first.")
    ...
    async def _assess_symbol_risk(self, symbol: str, current_price: float) -> Dict:
        ...
        try:
            ...
            risk_metrics = {
                'position_concentration': position_size * current_price / self.account_size if self.account_size > 0 else 0,
    ...
    async def _get_portfolio_risk_status(self) -> Dict:
        ...
        try:
            ...
            risk_status = {
                ...
                'risk_level': 'LOW' if total_exposure / self.account_size < 0.5 else 'MEDIUM' if total_exposure / self.account_size < 0.8 else 'HIGH'
            }
    ...
    async def get_current_price(self, symbol: str) -> float:
        ...
        try:
            ...
            return float(quote.get('bid', 0))  # Use bid price for conservative estimates
        except Exception as e:
    ...
    async def health_check(self) -> Dict:
        ...
        try:
            ...
            if self._alpaca_client:
                account = await self._alpaca_client.get_account()
                health_status['components']['alpaca'] = 'connected' if account else 'error'
            else:
                health_status['components']['alpaca'] = 'not_initialized'
            
            # Check other components
            health_status['components']['risk_manager'] = 'ready' if self._risk_manager else 'not_initialized'
            health_status['components']['order_manager'] = 'ready' if self._order_manager else 'not_initialized'
            health_status['components']['markov_system'] = 'ready' if self._markov_system else 'not_initialized'
