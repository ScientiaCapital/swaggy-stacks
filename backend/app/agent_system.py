...
Usage:
    ...
    consensus = await agent_system.request_consensus(
        "TSLA", context, ["market_analyst", "risk_advisor"]
    )
"""
...
from app.events.agent_event_bus import agent_event_bus
from app.events.multi_agent_coordinator import (
    ConsensusMethod,
    multi_agent_coordinator,
)
from app.testing.agent_testing_framework import (
    agent_testing_framework,
)

# Testing components
from app.testing.mock_data_generator import (
    MarketRegime,
    mock_data_generator,
)
...
class AgentSystem:
    ...
    async def _setup_integrated_callbacks(self):
        ...
        async def stream_coordination(coordination_update):
            ...
            await self.websocket_manager.broadcast_coordination_message(
                AgentCoordinationMessage(
                    sender_agent_id=coordination_update.get(
                    ...
                    requires_response=coordination_update.get(
    ...
    async def run_real_time_analysis(
        ...
        if markov_analysis is None:
            ...
            )  # Could be determined from market_data
    ...
    async def run_agent_tests(
        self, regime_filter: Optional[List[str]] = None, agent_types: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        performance_reports = await self.testing_framework.run_comprehensive_agent_test(
            regime_filter=regime_filter, agent_types=agent_types
        )
        ...
        return {
            "performance_reports": {
                k: v.__dict__ for k, v in performance_reports.items()
            },
    ...
    async def simulate_market_stream(
        ...
        logger.info(
            "Starting market stream simulation",
            symbol=symbol,
            regime=regime.value,
...
async def run_real_time_analysis(
    symbol: str, market_data: Dict[str, Any], **kwargs
) -> Dict[str, Any]:
...
async def request_agent_consensus(
    symbol: str, context: Dict[str, Any], **kwargs
) -> str:
