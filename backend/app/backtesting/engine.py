...
from app.backtesting.performance_analyzer import PerformanceAnalyzer, BacktestResult
...
class RefactoredBacktestEngine:
    ...
    async def single_instrument_backtest(
        ...
        logger.info(f"📊 Starting single instrument backtest: {symbol} with {strategy_name}")
        ...
        if self.config.enable_portfolio_correlation:
            result.correlation_matrix = self.performance_analyzer.calculate_correlation_matrix([symbol], data)
            result.performance_attribution = self.performance_analyzer.calculate_performance_attribution(
    ...
    async def multi_instrument_backtest(
        ...
        if self.config.enable_portfolio_correlation and len(symbols) > 1:
            result.correlation_matrix = self.performance_analyzer.calculate_correlation_matrix(symbols, data)
            result.performance_attribution = self.performance_analyzer.calculate_performance_attribution(
    ...
    async def strategy_comparison_backtest(
        ...
        logger.info(f"📊 Strategy comparison: {len(strategies)} strategies on {len(symbols)} symbols")
        ...
        if len(results) > 1:
            ...
            logger.info(f"🏆 Best strategy: {comparison.get('summary', {}).get('best_overall_strategy', 'Unknown')}")
    ...
    async def _execute_strategy_backtest(self, symbol: str, data, strategy_name: str):
        ...
        for i in range(50, len(data)):  # Start after lookback period
            ...
            # Generate signals
            all_signals = await self.signal_generator.generate_integrated_signals(symbol, market_data)
    ...
    async def _execute_portfolio_backtest(self, symbols: List[str], data: Dict, strategy_name: str):
        ...
        for i, date in enumerate(timeline):
            ...
            for symbol in symbols:
                ...
                await self._execute_portfolio_trade(symbol, all_signals, data, date, strategy_name)

    async def _execute_portfolio_trade(self, symbol: str, all_signals: Dict[str, Any],
                                     data: Dict, date: datetime, strategy_name: str):
        ...
        correlation_adjustment = self.portfolio_manager.calculate_correlation_adjustment(symbol, current_price)
    ...
    async def _execute_signal_based_backtest(self, symbol: str, data, signals: List[Dict[str, Any]]):
    ...
    def _extract_strategy_signal(self, all_signals: Dict[str, Any], strategy_name: str) -> Tuple[str, float]:
    ...
    def _patterns_to_signal(self, patterns: Dict) -> Tuple[str, float]:
        ...
        for pattern_name, pattern_data in patterns.items():
            if isinstance(pattern_data, dict) and pattern_data.get('detected', False):
                ...
                if 'bullish' in pattern_name.lower() or pattern_name.lower() in ['hammer', 'doji', 'morning_star']:
                    bullish_strength += strength
                elif 'bearish' in pattern_name.lower() or pattern_name.lower() in ['hanging_man', 'shooting_star', 'evening_star']:
    ...
    # Caching methods
    def _get_cached_result(self, backtest_id: str, symbols: List[str]) -> Optional[BacktestResult]:
    ...
    def get_historical_data(self, symbols: List[str], years: int = 3) -> Dict[str, pd.DataFrame]:
