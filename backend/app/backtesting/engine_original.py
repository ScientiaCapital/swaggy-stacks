...
class BacktestEngine:
    ...
    def __init__(self, config: BacktestConfig = None):
        ...
        logger.info(
            "🚀 BacktestEngine initialized with all strategies and enterprise features"
        )
    ...
    def _initialize_strategy_integrations(self):
        ...
        try:
            ...
            logger.info(
                "✅ Integrated Alpha Detector initialized (Elliott Wave, Fibonacci, Golden Zone, Wyckoff, Alpha Patterns)"
            )
        except Exception as e:
        ...
        try:
            ...
            logger.info(
                "✅ Candlestick pattern detection initialized (Pin Bar, Engulfing, Doji, Morning/Evening Star, Harami, Tweezers)"
            )
        except Exception as e:
    ...
    def _initialize_risk_management(self):
        ...
        try:
            ...
            risk_config = {
                "base_risk": {
                    ...
                    * self.config.initial_capital,
                    "max_daily_loss": self.config.initial_capital
                    * 0.02,  # 2% max daily loss
                },
                "position_sizing": {
                    "kelly_max_fraction": 0.25,
                    "fixed_fraction_default": 0.02,
                    "min_position_size": self.config.initial_capital
                    * 0.001,  # 0.1% minimum
                },
                "stop_loss": {
                    "atr_periods": 14,
            ...
            logger.info(
                "✅ Integrated Risk Management initialized (Kelly Criterion, ATR stops, VaR, Correlation limits)"
            )
        except Exception as e:
    ...
    async def generate_integrated_signals(
        ...
        if self.integrated_alpha_detector:
            try:
                alpha_signal = (
                    await self.integrated_alpha_detector.detect_alpha_opportunity(
                ...
                logger.warning(
                    f"IntegratedAlpha signal generation failed for {symbol}: {e}"
                )
        ...
        if df_data is not None and len(df_data) >= 20:
            try:
                ...
                modern_signal = self._generate_modern_indicator_signal(
                    modern_indicators, symbol
                )
                if modern_signal:
                    signals["modern_indicators"] = modern_signal
                    signal_scores["modern_indicators"] = modern_signal.get(
                        "signal_strength", 0.0
                    )

            except Exception as e:
                logger.warning(
                    f"Modern indicators calculation failed for {symbol}: {e}"
                )
                ...
                if self.llm_predictor:
                    ...
                    if llm_prediction:
                        signals["llm_predictions"] = {
                            "ensemble_prediction": llm_prediction.get(
                                "ensemble_prediction"
                            ),
                            "confidence": llm_prediction.get("confidence", 0.0),
                            "individual_predictions": llm_prediction.get(
                                "model_predictions", {}
                            ),
                            "reasoning": llm_prediction.get("reasoning", ""),
                            "risk_assessment": llm_prediction.get(
                                "risk_assessment", {}
                            ),
                        }
                        signal_scores["llm_predictions"] = (
                            self._extract_llm_signal_score(llm_prediction)
                        )
        ...
        for strategy_name, agent in self.strategy_agents.items():
            try:
                ...
                logger.warning(
                    f"{strategy_name} strategy signal failed for {symbol}: {e}"
                )
        ...
        if self.pattern_tool:
            try:
                ...
                if len(prices) >= 4:  # Minimum for pattern detection
                    patterns = self.pattern_tool._detect_candlestick_patterns(
                        prices, volumes
                    )
                    signals["candlestick_patterns"] = patterns
                    signal_scores["candlestick_patterns"] = (
                        self._extract_pattern_signal_score(patterns)
                    )
            except Exception as e:
                logger.warning(
                    f"Candlestick pattern detection failed for {symbol}: {e}"
                )
    ...
    def _generate_modern_indicator_signal(
        ...
        try:
            ...
            for indicator_name, values in indicators.items():
                if isinstance(values, dict) and "values" in values:
                    latest_values = values["values"][
                        -5:
                    ]  # Use last 5 values for trend analysis
                    ...
                    if "kama" in indicator_name.lower():
                        ...
                        if len(latest_values) >= 2:
                            ...
                            signal_strengths.append(
                                abs(latest_values[-1] - latest_values[-2])
                                / latest_values[-2]
                            )
            ...
            if signals:
                weighted_signal = (
                    np.average(signals, weights=signal_strengths)
                    if signal_strengths
                    else np.mean(signals)
                )
                ...
                signal_direction = (
                    ...
                    else ("BEARISH" if weighted_signal < -0.1 else "NEUTRAL")
                )
            ...
            logger.warning(
                f"Failed to generate modern indicator signal for {symbol}: {e}"
            )
            return None
    ...
    def _extract_pattern_signal_score(self, patterns: Dict[str, Any]) -> float:
        ...
        try:
            ...
            for pattern_name, pattern_data in patterns.items():
                if isinstance(pattern_data, dict) and pattern_data.get(
                    "detected", False
                ):
    ...
    def _combine_signals_with_weights(
        self, signal_scores: Dict[str, float], all_signals: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for signal_name, score in signal_scores.items():
                if (
                    signal_name not in self.signal_weights
                    and signal_name in all_signals
                ):
    ...
    async def single_instrument_backtest_with_strategy(
        self,
        symbol: str,
        ...
        logger.info(
            f"📊 Starting single instrument backtest: {symbol} with {strategy_name} strategy"
        )
        ...
        if custom_signals:
            ...
            await self._execute_strategy_backtest_with_signals(
                symbol, symbol_data, strategy_name
            )
        ...
        logger.info(
            f"✅ Single instrument backtest with {strategy_name} completed in {execution_time:.2f}s"
        )

        return result

    async def _execute_strategy_backtest_with_signals(
        self, symbol: str, data: pd.DataFrame, strategy_name: str
    ):
        ...
        for i in range(50, len(data)):  # Start after sufficient lookback period
            ...
            if (
                strategy_name == "integrated_alpha"
                and "integrated_alpha" in all_signals
            ):
    ...
    async def strategy_comparison_backtest(
        ...
        logger.info(
            f"📊 Starting strategy comparison backtest: {len(strategies)} strategies on {len(symbols)} symbols"
        )
        ...
        for strategy in strategies:
            ...
            try:
                if len(symbols) == 1:
                    result = await self.single_instrument_backtest_with_strategy(
                        symbols[0], strategy
                    )
                else:
                    result = await self.multi_instrument_backtest_with_strategy(
                        symbols, strategy
                    )
    ...
    async def multi_instrument_backtest_with_strategy(
        ...
        await self._execute_portfolio_backtest_with_strategy(
            symbols, data, strategy_name
        )
        ...
        execution_time = time.time() - start_time
        logger.info(
            f"✅ Multi-instrument backtest with {strategy_name} completed in {execution_time:.2f}s"
        )

        return result

    async def _execute_portfolio_backtest_with_strategy(
        self, symbols: List[str], data: Dict[str, pd.DataFrame], strategy_name: str
    ):
        ...
        for i, date in enumerate(timeline):
            ...
            for symbol in symbols:
                ...
                # Generate signals
                all_signals = await self.generate_integrated_signals(
                    symbol, market_data
                )
    ...
    async def _execute_portfolio_trade_with_strategy(
        self,
        symbol: str,
    ...
    async def single_instrument_backtest(
        self, symbol: str, strategy_signals: List[Dict[str, Any]] = None
    ) -> BacktestResult:
        """Execute backtest for single instrument (backwards compatibility)"""
        return await self.single_instrument_backtest_with_strategy(
            symbol, "integrated_alpha", strategy_signals
        )

    async def multi_instrument_backtest(
        self,
        symbols: List[str],
        strategy_signals: Dict[str, List[Dict[str, Any]]] = None,
    ) -> BacktestResult:
        ...
        if strategy_signals:
            ...
            return await self.multi_instrument_backtest_with_strategy(
                symbols, "integrated_alpha"
            )

    async def multi_trade_per_signal_backtest(
        self, symbols: List[str], trade_count_per_signal: int = 3
    ) -> BacktestResult:
        """Execute multiple trades per signal with position pyramiding"""
        start_time = time.time()
        backtest_id = (
            f"multi_trade_{len(symbols)}_{trade_count_per_signal}_{int(start_time)}"
        )

        logger.info(
            f"📊 Multi-trade backtest: {trade_count_per_signal} trades per signal"
        )
    ...
    async def _execute_strategy_backtest(
        self,
        symbol: str,
        data: pd.DataFrame,
        ...
        if strategy_signals:
            ...
            await self._execute_strategy_backtest_with_signals(
                symbol, data, "integrated_alpha"
            )

    async def _execute_signal_based_backtest(
        self, symbol: str, data: pd.DataFrame, signals: List[Dict[str, Any]]
    ):
    ...
    async def _execute_markov_based_backtest(self, symbol: str, data: pd.DataFrame):
        ...
        await self._execute_strategy_backtest_with_signals(
            symbol, data, "markov_traditional"
        )
    ...
    def enhance_backtest_results_with_risk_metrics(
        ...
        try:
            if not self.risk_manager:
                logger.warning(
                    "Risk manager not initialized, skipping risk metrics enhancement"
                )
                return base_results
            ...
            daily_pnl = 0.0
            if (
                "portfolio_value_history" in base_results
                and len(base_results["portfolio_value_history"]) > 1
            ):
                recent_values = base_results["portfolio_value_history"][-2:]
                daily_pnl = recent_values[-1].get("value", 0) - recent_values[-2].get(
                    "value", 0
                )
            ...
            for symbol in base_results.get("symbols", []):
                if symbol in price_history and price_history[symbol]:
                    ...
                    optimal_position, sizing_details = (
            ...
            for symbol in base_results.get("symbols", []):
                if symbol in price_history and len(price_history[symbol]) >= 14:
                    ...
                    market_data = {
                        ...
                        ],  # Mock highs (1% above close)
                        "lows": [
                            p * 0.99 for p in prices
                        ],  # Mock lows (1% below close)
                        "closes": prices,
                    }

                    # Calculate comprehensive stop loss
                    stop_price, stop_details = (
                        self.risk_manager.calculate_comprehensive_stop_loss(
                            symbol=symbol,
                            entry_price=current_price,
                            side="BUY",
                    ...
                    stop_loss_analysis[symbol] = {
                        ...
                        "stop_distance_pct": abs(stop_price - current_price)
                        / current_price,
            ...
            enhanced_results = {
                **base_results,
                "risk_management": {
                    ...
                    "correlation_analysis": risk_assessment.get(
                        "correlation_analysis", {}
                    ),
                    "sector_analysis": risk_assessment.get("sector_analysis", {}),
                },
                "risk_adjusted_metrics": {
                    "risk_adjusted_return": base_results.get("total_return", 0)
                    / max(risk_assessment.get("risk_score", 50) / 100, 0.1),
                    "var_utilization": risk_assessment.get("var_analysis", {}).get(
                        "var_pct_of_portfolio", 0
                    ),
                    "portfolio_heat": sum(
                        sizing["kelly_fraction"]
                        for sizing in position_sizing_analysis.values()
                        if "kelly_fraction" in sizing
                    ),
                    "diversification_ratio": 1
                    - risk_assessment.get("correlation_analysis", {}).get(
            ...
            logger.error(
                "Error enhancing backtest results with risk metrics", error=str(e)
            )
            # Return base results if enhancement fails
            return {**base_results, "risk_management_error": str(e)}
