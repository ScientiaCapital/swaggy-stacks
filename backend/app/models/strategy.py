...
class Strategy(Base):
    ...
    strategy_type = Column(String(50), nullable=False)  # MARKOV, FIBONACCI, ELLIOTT_WAVE, WYCKOFF
    ...
    def profit_factor(self) -> float:
        ...
        return float(self.winning_trades / self.losing_trades) if self.losing_trades > 0 else 0
    
    @property
    def average_win(self) -> float:
        """Calculate average win amount"""
        return float(self.total_pnl / self.winning_trades) if self.winning_trades > 0 else 0
    
    @property
    def average_loss(self) -> float:
        """Calculate average loss amount"""
        return float(self.total_pnl / self.losing_trades) if self.losing_trades > 0 else 0
