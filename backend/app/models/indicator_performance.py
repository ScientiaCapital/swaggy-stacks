...
class IndicatorPerformance(Base):
    ...
    market_condition = Column(String(20), nullable=True)  # volatile, trending_up, trending_down, sideways
    ...
    __table_args__ = (
        ...
        UniqueConstraint('indicator_name', 'indicator_type', 'period_start', 'period_end', 'market_condition',
...
class MLModelVersion(Base):
    ...
    model_type = Column(String(50), nullable=False)  # LLM, LSTM, GRU, TRANSFORMER, ENSEMBLE
    ...
    # Model configuration
    model_config = Column(JSON, nullable=False)  # Store model architecture, hyperparameters
    training_config = Column(JSON, nullable=True)  # Training parameters, data preprocessing

    # For LLM models
    llm_provider = Column(String(50), nullable=True)  # ollama, openai, anthropic
    llm_model_id = Column(String(100), nullable=True)  # qwen2.5, yi-6b, glm-4-9b, deepseek
    ...
    optimization_results = relationship("ParameterOptimization", back_populates="model_version")
...
class MLPrediction(Base):
    ...
    # Prediction values
    predicted_direction = Column(String(10), nullable=False)  # BULLISH, BEARISH, NEUTRAL
    predicted_return = Column(Numeric(10, 6), nullable=True)
    confidence_score = Column(Numeric(5, 4), nullable=False)

    # Ensemble details (for multi-model predictions)
    individual_predictions = Column(JSON, nullable=True)  # Store each model's prediction
...
class IndicatorParameters(Base):
    ...
    market_condition = Column(String(20), nullable=True)  # Can be null for general parameters
    ...
    optimization_method = Column(String(50), nullable=True)  # grid_search, bayesian, genetic
    optimization_metric = Column(String(50), nullable=True)  # sharpe_ratio, total_return, win_rate
    ...
    previous_version_id = Column(Integer, ForeignKey("indicator_parameters.id"), nullable=True)
    ...
    # Indexes
    __table_args__ = (
        Index('ix_indicator_parameters_lookup', 'indicator_name', 'market_condition', 'is_active'),
        Index('ix_indicator_parameters_hash', 'parameter_hash'),
        UniqueConstraint('indicator_name', 'market_condition', 'parameter_hash',
...
class ParameterOptimization(Base):
    ...
    # Optimization target
    indicator_name = Column(String(100), nullable=True)  # Can optimize multiple indicators
    model_version_id = Column(Integer, ForeignKey("ml_model_versions.id"), nullable=True)
    optimization_type = Column(String(50), nullable=False)  # indicator, model, ensemble

    # Optimization configuration
    method = Column(String(50), nullable=False)  # grid_search, bayesian, genetic, random
    ...
    # Relationships
    model_version = relationship("MLModelVersion", back_populates="optimization_results")
    created_parameters = relationship("IndicatorParameters",
