...
class MCPServerDependency:
    ...
    async def __call__(
        self, orchestrator: MCPOrchestrator = Depends(require_mcp_available)
...
require_sequential_thinking = MCPServerDependency(
    MCPServerType.SEQUENTIAL_THINKING, required=True
)
require_github = MCPServerDependency(MCPServerType.GITHUB, required=True)
require_shrimp_task_manager = MCPServerDependency(
    MCPServerType.SHRIMP_TASK_MANAGER, required=True
)

# Optional server dependencies (won't raise error if unavailable)
optional_taskmaster_ai = MCPServerDependency(
    MCPServerType.TASKMASTER_AI, required=False
)
...
optional_sequential_thinking = MCPServerDependency(
    MCPServerType.SEQUENTIAL_THINKING, required=False
)
optional_github = MCPServerDependency(MCPServerType.GITHUB, required=False)
optional_shrimp_task_manager = MCPServerDependency(
    MCPServerType.SHRIMP_TASK_MANAGER, required=False
)
...
async def mcp_server_connection(
    server_type: MCPServerType, orchestrator: MCPOrchestrator
):
    """
    ...
    try:
        ...
        logger.error("MCP connection error", server=server_type.value, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"Connection to {server_type.value} failed",
        )
    except Exception as e:
        logger.error("Unexpected MCP error", server=server_type.value, error=str(e))
        raise HTTPException(
...
async def validate_server_availability(
    server_types: list[MCPServerType], orchestrator: MCPOrchestrator
...
async def get_server_status_summary(orchestrator: MCPOrchestrator) -> Dict[str, Any]:
    """
