...
from app.models.indicator_performance import IndicatorPerformance, MLPrediction
import pandas as pd
import numpy as np
...
class IndicatorCalculationRequest(BaseModel):
    ...
    indicator_type: str = Field(default="modern", description="Type: traditional, modern, or both")
    specific_indicators: Optional[List[str]] = Field(None, description="Specific indicators to calculate")
...
class PerformanceQueryRequest(BaseModel):
    ...
    indicator_type: Optional[str] = Field(None, description="Filter by type: TRADITIONAL, MODERN, LLM")
    market_condition: Optional[str] = Field(None, description="Market condition filter")
    lookback_days: Optional[int] = Field(default=90, description="Lookback period in days")


class OptimizationRequest(BaseModel):
    """Request for parameter optimization"""
    indicator_name: str = Field(..., description="Indicator to optimize")
    parameter_space: Dict[str, Dict[str, Any]] = Field(..., description="Parameter search space")
    optimization_metric: str = Field(default="sharpe_ratio", description="Optimization target metric")
    max_iterations: int = Field(default=50, ge=1, le=200, description="Maximum optimization iterations")
    market_condition: Optional[str] = Field(None, description="Optimize for specific market condition")
...
async def calculate_modern_indicators(request: IndicatorCalculationRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if market_data.timestamps:
            data_dict['timestamp'] = pd.to_datetime(market_data.timestamps[:len(market_data.prices)])
        ...
        indicator_type = IndicatorType.MODERN if request.indicator_type == "modern" else IndicatorType.BOTH
        ...
        if request.specific_indicators:
            indicators = {k: v for k, v in indicators.items()
                         if k in request.specific_indicators}
...
async def get_llm_predictions(request: LLMPredictionRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not llm_predictor:
            raise HTTPException(status_code=503, detail="LLM prediction service unavailable")
        ...
        raise HTTPException(status_code=500, detail=f"Error generating LLM predictions: {str(e)}")


@router.get("/indicators/performance/{indicator_name}")
async def get_indicator_performance(
    indicator_name: str,
    market_condition: Optional[str] = Query(None, description="Market condition filter"),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving indicator performance: {str(e)}")
...
async def get_market_condition_analysis(
    market_condition: Optional[str] = Query(None, description="Specific market condition")
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing market conditions: {str(e)}")
...
async def update_signal_outcome(signal_id: str, actual_outcome: Dict[str, Any]):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error updating signal outcome: {str(e)}")


# Background task functions
async def track_prediction_performance(symbol: str, prediction_result: Dict[str, Any], horizon_days: int):
...
async def run_parameter_optimization(
    ...
    try:
        ...
        print(f"Starting optimization for {indicator_name} with {max_iterations} iterations")
